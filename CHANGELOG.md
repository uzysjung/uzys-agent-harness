...
- **트랙이 자동으로 고르는 자산이 위저드에 전부 보이고 해제할 수 있다** (ADR-074). 전에는 트랙을
  고르면 32개가 조용히 딸려 왔다 — 무엇이 깔리는지 화면에 없었다.
...
- **배포물 셸 건전성 게이트**(저장소 내부). 배포될 `gh` 명령을 줄이음(`\`) 뒤에 주석을 붙여
  ...
  때문이다 — `bash -n` 은 그 파손을 **문법상 합법으로 통과시킨다**(실측). 배포되는 `.sh` **18개 중 16개는
  어떤 검사도 없었고**, 나머지 2개도 기존 테스트가 *실행이 도달하는 코드만* 간접 확인했다 —
  도달하지 않는 구간의 문법 오류는 18개 전부 무방비였다.
