...
### Changed
- **`clear-korean-communication` → `user-centered-explanation`** (#485 · #494, 사용자 결정): 이름이 "한국어 전용"으로
  읽혀 영어 사용자에게 닿지 않았다. 본문은 사용자가 이슈에 첨부한 개정안 — 문제·변경안·선택지를 사용자 행동·결과로
  재구성하는 예시, 제안/구현/검증/배포 상태 구분, 화면·동작 전후 비교(텍스트 와이어프레임) 절, "형식 충족보다
  이해로 평가"하는 품질 기준. 답변 언어는 사용자의 언어. 기존 설치본은 update 의 `new-skills` 묶음이 새 이름을
  깔고, 옛 디렉터리는 남기되 화면이 "지워도 된다"고 말한다(옛 문구 "선택해 받아라"는 v26.154.0 이후 거짓이라 고쳤다).
- **상시 한 줄을 사용자 원칙 문안으로** (#493 · #494): 설치기가 깐 상시 스킬에 대해 프로젝트 맥락 블록(4 CLI)에 쓰는
  한 줄이 "내부 구현보다 사용자 행동과 결과를 먼저 — 재설명·UX 전후·중요한 선택 정리는 스킬로"가 됐다. 스킬을
  부르지 않은 평소 보고에도 관점이 실린다. 상주 비용(tooling): 지시문 ~3,506 → ~3,537 · 합계 ~5,648 → ~5,691.
- **`recurrence-prevention` 대책의 형태** (#451 · #495, 사용자 방향 2026-09-14): 사다리는 "얼마나 세게"만 말했고
  "어떤 형태"는 말하지 않아 대책이 금지문·고정 절차·매번 도는 검사로 쌓였다. 이제 Level 1 = 실패를 막는 최소
  조건을 긍정형으로, Level 2 = 더 결정론적이지 더 많은 금지가 아니다 · pre-flight 넷째 질문(판단을 대신하나,
  입력을 주나) · 룰 템플릿 "절대 원칙" → "지켜야 할 상태" · 후보 비교 축에 매번 내는 속도 비용과 자율 영향 ·
  Pitfalls "금지문 축적" · 장치 표에 Per-run cost · Autonomy impact 열.
...
백업처리해버리는 부분에서 스트레스 받는다."* 이번 판의 update 는 ⓐ 무엇을 갱신할지 고를 수 있고
ⓑ 고친 앵커를 조용히 잃지 않으며 ⓒ 릴리즈로 새로 생긴 스킬을 기존 설치본에도 주고 ⓓ 백업이 생기면
목록과 다음 행동을 알려 준다. 설계 방향은 사용자가 정했다(#480): 룰·앵커는 백업하고 교체하되, 그 뒤
audit-harness-fit 이 편집분을 다시 얹을 수 있게 **증거는 하네스가 낸다** — 범용 스킬에 이 하네스의
백업 규약을 심지 않는다.

### Added
- **update 항목 선택** (#480 ① · #483): 여섯 묶음(`skills` · `new-skills` · `rules` · `anchor` · `hooks` · `external`)을
  `agent-harness update --only <group>`(반복)과 위저드 체크박스(기본 전부 체크)로 고른다. 안 고른 묶음은 한 바이트도
  안 건드리고 `⊘ not selected — untouched` 행으로 남긴다. 기준선(룰·스킬 sha)은 동기화한 디렉터리만 다시 찍는다 —
  건너뛴 디렉터리의 편집을 "하네스 소유"로 오기록하면 다음 update 가 백업 없이 덮어쓴다(독립 리뷰가 실증한
  두 방향의 함정을 둘 다 막는다). 모르는 `--only` 값은 exit 1.
- **새로 생긴 번들 스킬 설치** (#480 ①): 릴리즈가 추가한 스킬이 기존 설치본에 영영 오지 않던 것 — `new-skills` 묶음이
  이 트랙 구성의 기본 선택 중 디스크에 없는 것을 깐다. 설치 때 해제한 것은 ADR-074 대로 되살리지 않는다
  (E2E 가 첫 구현의 위반을 잡았다).
- **백업 목록 + 다음 행동** (#480 ③ · #487): 이번 update 가 남긴 원본↔백업 쌍을 `.uzys-agent-harness/update-backups.json` 에
  쓰고, 요약 마지막에 `BACKUPS`(목록 경로)·`NEXT`(audit-harness-fit 에 "다시 얹어줘" 요청) 두 줄을 낸다. 백업이 없으면
  파일도 없다.
- **위저드 행 설명** (#421 ① · #484): 3단계의 룰·에이전트·훅·스킬 행에 그 파일에서 뽑은 한 줄(룰 = 제목 + 첫 문장 ·
  에이전트·스킬 = frontmatter description · 훅 = 머리 주석)이 붙는다. 파일이 바뀌면 같이 바뀐다.
- **방법론 스킬 개별 행** (#421 ② · #486 · ADR-092): "uzys 하네스 방법론 5종" 한 행을 다섯 행으로 풀고 각자 설명을 붙였다 —
  하나만 뺄 수 있다. 기본 체크는 다섯 전부(사용자 결정). ADR-028 ⓐ 대체, ⓑ 페이지 분할·행수 상한 게이트는 유지.

### Fixed
- **고친 앵커가 update 에서 백업 없이 사라지던 것** (#480 ② · #482): `CLAUDE-uzys-harness.md` 를 룰·스킬과 같은
  잣대(설치 시점 sha 와 다르면 `*.backup-<time>`)로 다룬다. 갱신 뒤 기준선을 다시 찍어 편집 없는 재실행은 백업을
  만들지 않고, 기준선 없는 레거시 설치본은 보수적으로 백업한다. 화면에 `edited` 행.
- **번들에서 사라진 스킬 파일이 update 뒤에도 남던 것** (#477 · #479): 26.152.0 → 26.153.0 에서
  `clear-korean-communication/references/` 옛 3파일이 새 SKILL.md 옆에 남았다. 번들 스킬 디렉터리의 파일 목록이
  최신본의 정의다 — 목록 밖 파일은 지우되 사용자 것(기준선 없음·다름)은 먼저 백업한다. 스킬 기준선은 번들에 있는
  파일만 담는다(독립 리뷰 HIGH — install 재실행 경로에서 사용자 파일이 백업 없이 지워지던 것). ADR-046 표 행 추가.
- **큰 npm 자산 설치가 "멈춤"으로 보이던 것** (#422 · #481): `netlify-cli`(413 MB, 컨테이너 실측 86초) 같은 npm·npx
  자산은 결과 행이 뜰 때까지 화면이 침묵했고 120초에 끊겼다. 시작 시 진행 줄을 내고 상한을 10분으로, 초과하면
  직접 이어 돌릴 명령을 그대로 낸다.
- **비-claude 단독 설치의 상주 계측이 깔리지 않는 서브에이전트를 세던 것** (#476 · #478): Codex/OpenCode/Antigravity
  단독 설치 헤더·wizard confirm 이 `agents 2 ~192` 를 더해 보였다(같은 설치의 update 화면은 0). 계획 엔트리 중 그
  CLI 조합에서 디스크에 남는 것만 센다 — 룰(AGENTS.md 인라인)·스킬(`.agents/skills/`)은 그대로.
- docker `scenario-update-skills` 가 위저드 update 의 새 체크박스 단계 때문에 update 를 아예 못 돌리던 것 —
  릴리즈 태그의 docker-scenarios 에서 빨개졌을 회귀를 #487 에서 잡았다(pty 키 열에 Enter 추가).

### Docs
- ADR-092 신설(wizard 방법론 개별 행) · ADR-028 ⓐ Superseded · ADR-046 표에 "번들에 없는 스킬 파일" 행.
- #415(앵커 A/B) 닫음 — 재려던 블록이 #463 에서 통째로 교체돼 대상이 없다. #442 를 well-known 엔드포인트 호스팅으로
  정의(착수는 Vercel 프로젝트·도메인 확보 뒤). #454 에 "audit-harness-fit 완성 뒤 테스트 효율 재점검" 지시와 오늘의
  리뷰 사례 2건 기록.

## [v26.153.0] — 2026-09-20 (결과와 경계는 고정, 방법은 모델이 고른다 — 사용자 문안으로 지침 4종 개정)

사용자가 이슈에 첨부한 문안 4종을 축자로 넣었다(#463 · #464 · #465 · #462, ADR-091). 방향은 하나다 —
**달성할 결과와 지켜야 할 경계만 분명히 하고, 그 안에서 방법은 모델이 고른다.** 승인·권한·시크릿·
사용자 작업 보호 경계는 그대로다. 그 밖에 v26.152.0 이후 잡힌 결함 4건.

### 설치받는 사람에게 달라지는 것

- **배포 앵커(`CLAUDE-uzys-harness.md` · `AGENTS.md` · Antigravity 룰)가 Working Principles 문안이 된다**(#463).
  "머지·배포 전 독립 리뷰 필수" 게이트 대신 검증 깊이·시점·독립 리뷰 여부를 영향·불확실성·복구 비용에
  따라 고른다. 전역 `~/.claude/CLAUDE.md` 와의 바이트 동일 계약(ADR-085)은 끝났다 — 앵커 문안은 이
  리포에 이슈로 준다.
- **`model-orchestration` 이 자율 배분 원칙 6절이 된다**(#464). 고정 모델 역할표 · effort 하한 ·
  외부 실행기 5술어 · anti-pattern 표가 빠진다. 한 컨텍스트에서 끝내는 것이 정당한 선택이고, 추가
  워커·모델 호출은 기대 기여가 비용을 넘을 때만. 제3자 제공자에 저장소를 보내기 전 승인은 여전히 필요.
- **`/clear-korean-communication` 만 부르면 기본 표를 받는다**(#465) — 맥락 · 문제점 · 해결방안 ·
  추천방안(각각 이유). 매번 형식을 덧붙여 부를 필요가 없다. 본문이 가리키지 않게 된 `references/`
  3파일은 뺐다.
- **`audit-harness-fit` 3판**(#462) — 감사 목적이 "불필요한 지침 제거"에서 "성과를 높이는 지침
  설계"로. 고정 검증·커밋 절차를 완화하고, 제안 → 제한적 시험 → 채택 경로, 위임은 실행 경로 선택,
  보고 깊이는 영향에 비례. 상시 감사 훅·전역 리뷰 게이트·승인 루프는 만들지 않는다는 경계가 본문에 있다.
- **`update` 화면이 트랙 밖으로 강등된 에이전트를 말한다**(#458) — `data-analyst` · `strategist` 가
  남은 tooling 설치본에 트랙명과 "지워도 된다" 한 줄. 상주 계측은 갱신 뒤 디스크를 센다(20 → 22 처럼
  실제 값). 파일은 지우지 않는다.
- **Codex 설치본이 없는 훅을 부르지 않는다**(#438) — `.codex/config.toml` 의 `post_tool_use` 가 존재하지
  않는 `uncommitted-check.sh` 를 매 Bash 호출 뒤에 부르던 배선 제거. 설치되는 훅은 `session-start` 하나.
- **consult 래퍼가 CLI 종료 뒤 1초를 놀지 않는다**(#411) — 이식형 timeout 의 폴링 1초 → 200 ms. 이
  리포의 릴리즈 CI 가 이유 없이 red 가 되던 원인이기도 했다(케이스당 ~1.3초 → ~0.5초).
- **`taste-skill` · `preline` 설치가 다시 된다**(#420) — 업스트림이 스킬 `name:` 을
  `design-taste-frontend` · `preline-theme-generator` 로 바꿨고 skills CLI 는 그 이름으로 맞춘다.
- **상주 비용(tooling)**: **20개 ~6,546** → **20개 ~5,648** tokens/세션 — 지시문 ~4,225 → **~3,506**
  (앵커 −719) · 발화 표면 ~2,321 → ~2,142(ckc −198 · audit-harness-fit +19).

### 이 리포

- ADR-091(Accepted) — ADR-085 결정 1 · ADR-069 amend. ADR-044 에 update agents 축의 디스크 예외 포인터.
- 앵커 도달 게이트: '설계 리뷰 분리' 축은 리포 앵커 전용으로, '검증의 자기 증거' 축은 새 문안 어휘 추가.
- 도커 시나리오 `update-demoted-agents` 신설(registry 의 실제 26.151.0 → 이 빌드로 update).
...
**설치자의 개발 속도를 늦추나 · 서로 충돌하나 · Fable 급 모델이 이 문장 없이 못 하는 것이 있나.**
문턱 · 게이트 · 보안 · 데이터 보호 · 배포 승인은 한 항도 약화하지 않았다.

### 설치받는 사람에게 달라지는 것

- **코드 리뷰가 수정마다 뜨지 않는다 — 사용자 씬이 완성됐을 때, 그리고 문턱 위 변경의 머지 전에만**
  (#424 · #423 · ADR-087). 리뷰 문턱이 세 층에 세 값으로 있었다(에이전트 descriptor *"MUST BE USED
  for all code changes"* · 앵커 "완료 선언 전" · Delivery 룰 "머지 전"). 가장 넓은 문으로 읽으면 CSS 한 줄에도
  리뷰 에이전트가 떴다. 이제 Delivery 룰이 **검증의 리듬**(구현 중 변경부 빠른 검사 → 씬 수정분 모아
  독립 검토 → 통합 · 빌드 · 실사용 흐름)과 **머지 전 독립 검증 문턱**(핵심 사용자 기능 · 되돌리기 어려운
  것 · 돈·권한)을 한 곳에서 정한다. 그 밖은 리그레션 테스트로 들어오고 배포 게이트가 마지막에 본다.
  **돈 · 권한 · 데이터/마이그레이션 · 되돌리기 어려운 조작은 씬 묶음과 별도의 독립 검증 대상**이다 —
  대표 여정 통과가 그것을 대신하지 못한다.
- **배포 앵커가 사용자의 전역 6원칙과 바이트 동일해졌다** (#427 · ADR-085). 그전에는 이 리포가
  쓴 7원칙 + 꼬리 2절이었다. 앵커에서 빠진 "상시 적용 스킬" 안내는 설치기가 프로젝트 블록에 **실제로
  깔린 스킬만** 적는다 — 4 CLI 전부. 의사결정 형식(ASIS→TOBE)은 `clear-korean-communication` 스킬이
  소유한다.
- **`audit-harness-fit` 2판** (#425 · ADR-084 · #441). 판정 근거가 "공식 문서 인용" 에서 **확정된
  의도 + 리포 실증**으로 바뀌었고, 감사 영역 5 가 **개발 속도를 늦추는 지시**(매 편집 전체 실행 · 매 변경
  리뷰 · 씬 하나로 묶을 수 있는 검사)를 finding 으로 잡는다. 검증 안내에 "씬이 완료되면 묶어서 Usage
  기반으로 한 번" 절이 들어갔다 — 묶음은 미룸이 아니다(씬 = 한 행위자 · 한 관측 결과, 커지면 쪼갠다,
  변경마다 커밋은 유지, 머지 · 배포 게이트는 그 자리). 설치 직후 한 번 돌리면 프로젝트 맥락을 리포에서
  읽어 채운다(populate).
- **번들 스킬이 Codex · OpenCode · Antigravity 에도 디렉터리 전체로 간다** (#431 · ADR-086). 그전에는
  `SKILL.md` 한 파일만 복사돼 references · scripts 를 가진 스킬 14종이 3/4 CLI 에서 빈 껍데기였다.
- **`task-brief` → `objective-brief`, 위임 · 설계 · 다단계 작업에만** (#426 D4 · ADR-088). 모든 요청을
  브리프로 만들어 보여주던 규율이 한 줄 요청에도 라운드를 늘렸다. 매 프롬프트 넛지 훅은 제거, 상시
  안내에서도 뺐다. 이름은 "objective" 가 뜻이다(`object-brief` 는 객체로 읽혀 기각).
...
- **리뷰 에이전트 2종 은퇴** (#445 · #443 · ADR-089): `code-reviewer` · `security-reviewer`(ECC 파생)는
  ...
  분석). **`reviewer` 는 남는다** — 씬 + 완료 기준을 받아 완료 판정을 내리는 독립 레인은 벤더 명령이
  대신하지 못한다. 기본 설치 에이전트 9 → 7종.
- **이미 깐 설치본의 `update` 가 개명 · 은퇴를 말한다**: 개명 id 는 *"`objective-brief` 가 됐다 — 선택해
  받는다"*, 은퇴 id 는 *"지워도 된다 (+ 대신 쓸 것)"*. 자동 삭제는 없다 — 단 설치 기준선(ADR-047)이 있는
  설치본의 은퇴 **에이전트** 파일은 기존 prune 이 회수한다(편집분은 백업).
- **Codex · OpenCode · Antigravity 의 `AGENTS.md` 껍데기에서 이 리포의 사정이 빠졌다** (#439): 없는
  스크립트명 · 이슈 번호 · 실측 날짜 · 깔리지 않는 에이전트 5종 표(`spawn_agent` 안내) · 앵커에도 룰에도
  없던 "즉시 commit" · 새 프로젝트엔 없는 `docs/SPEC.md` 를 매 세션 재참조하라는 지시. lock 파일은
  "보고만" 에서 **"손으로 고치지 않는다 — 패키지 매니저로 재생성"** 으로 — 의존성 추가를 막지 않는다.
  Codex 에는 SPEC 이 있을 때만 읽으라는 조건부 한 줄이 남는다(기본 설치는 `session_start` 훅이 trust
  entry 없이는 로드되지 않는다).
- **`protect-files` 훅이 `.env.example` · `.env.sample` · `.env.template` 을 막지 않는다** (2차 감사
  G-01). `.env.*` 글롭이 시크릿 없는 예시 파일까지 잡아 에이전트가 만들 때마다 BLOCKED 였다(실측). 실제
  환경 파일은 그대로 차단.
- **배포 룰에서 모델이 아는 방법 지식 3문장과 앵커와 겹치는 4문장을 뺐다** (G-02~G-06): Testing 의
  "관측 가능 행동" · "결정적 테스트" · "커버리지는 목표 아님", Delivery · Change Boundaries · Shell Safety ·
  Git Safety 의 앵커 중복. 문턱 · high-risk 정의 · CI 일정 · 개인정보 금지 · 단언 약화 금지는 그대로.
- **스킬 · 에이전트 본문 한 문단씩** (#440): `model-orchestration` 은 모델명 · effort 가 Claude Code
  어휘이고 다른 CLI 는 그 벤더의 최상위/중간 티어로 같은 역할 분리를 적용한다(Codex · Antigravity
  설치자가 정책 위반에 갇히지 않는다). `reviewer` 는 "함수 50줄 · 파일 800줄" 절대 숫자 대신 **이
  저장소의 관례**로 판단하고, 입력으로 diff 가 아니라 **씬과 완료 기준**을 받는다.
...
- 이 리포의 상주 파일(루트 `CLAUDE.md` · `.claude/CLAUDE.md` · 룰)에서 경위 · 전례 · 정정 이력 · 사례표
  행을 ADR · 계획 · 이슈 링크로 내렸다(#448, 28그룹 전량 목적지 실재 확인). 죽은 `mcp-pre-exec` 훅
  배선 삭제. 이 리포의 머지 전 독립 리뷰 문턱도 배포 룰과 같아졌다(설치자에게 나가는 것 · 릴리즈 배선 ·
  공유 상태만).
- reachability 게이트의 훑기 범위를 헤더가 약속한 수준으로(#414). 앵커 문단 삭제(#416)와 그 되돌림(#419)은
  순증 0.
- 후속 이슈: #429 사용자 문서 재작성(설치 직후 `audit-harness-fit` 1회 안내 포함) · #437 텍스트 단언
  테스트 점검 · #438 Codex `uncommitted-check` 죽은 배선 · #446 에이전트 7종 Fable 기준 감사.

## [v26.150.0] — 2026-08-30 (세지 못하던 스킬, 조용히 사라졌던 발화 문구, 0건만 내던 탐지기)

셋 다 **exit 0 이었고 화면에는 초록불이 떴다.** 틀렸다는 사실을 알 방법이 없었다.

### 설치받는 사람에게 달라지는 것

- **상주 비용 화면이 실제 설치와 맞고, 두 축으로 갈렸다** (#320 · ADR-083). 그전에는 한 숫자였고
  **번들 스킬을 세지 않아 약 32% 적게** 나왔다 — tooling 트랙 실측 **23개 ~5,331** → **34개 ~7,838
  tokens/세션**. 이제 설치 화면과 `cost:report` 가 **같은 배선(manifest)에서** 숫자를 뽑는다.
  더 중요한 것은 그 한 숫자가 **성질이 다른 둘을 섞고 있었다**는 점이다:
  - **지시문 8개 ~4,314** (rules 6 · CLAUDE.md 2) — 우리가 항상 읽히게 넣은 것.
  - **발화 표면 26개 ~3,524** (skill descriptor 17 · agent descriptor 9) — 스킬·에이전트가
    *불리기 위해* 상주시키는 descriptor. **깎으면 안 불린다.**

  둘 다 작을수록 좋은 값이 아니다. 지시문은 *불필요한 것이 있나*로 판단하고, 게이트는
  **조용한 증가만** 막는다.
- **스킬이 다시 불린다 — 소실됐던 발화 문구 7개 중 6개 복구** (#333 후속). descriptor 를 상한에 맞춰
  줄이는 과정에서 트리거 문구가 함께 잘려 나갔다. 길이는 줄었고 전 스위트는 초록이었다.
  이제 스킬별 상한을 baseline 에 등록하고 **등록이 없으면 빨간불**이다 — 줄을 지우는 것이
  영구 면제가 되던 구멍을 닫는다.
- **고아 프로세스 탐지가 서브에이전트를 보고, 훅이 그전보다 빨라졌다** (#326 · #400). 판정식이
  커맨드라인만 봐서, 프로젝트 경로가 cwd 에만 있는 서브에이전트를 **0건**으로 보고했다 —
  **0 을 내는 탐지기는 없는 것보다 나쁘다.** cwd 축을 더했다. 처음 판본은 같은 부하 A/B 교차
  실측에서 **중앙값 ~1,249 → ~3,304 ms (2.6배 회귀)** 였고 그것을 main 에 실어 보냈다 —
  릴리즈 전에 잡아 **설치받은 사람에게는 가지 않았다.** 비싼 것은 후보 수가 아니라 `lsof`
  자신이었고(후보 1개짜리가 1,444 ms), `-b` 를 붙이자 같은 답에 74 ms 가 됐다. 그래서 좁혔던
  후보 범위를 **오히려 다시 넓힐** 수 있었다. 출하되는 판본의 중앙값은 **~895 ms — 이 변경
  전(~1,249 ms)보다 빠르다.**
- **컴팩션 스킬의 기억 규칙이 크기 대신 적합성을 잰다** (#401). 200줄/20KB 한도만 재던 규칙이
  죽은 항목 48개를 통과시켰다. 그 색인은 **매 세션 상주**하므로 비용은 설치받은 사람이 내고, 더
  나쁘게는 죽은 항목이 살아 있는 사실처럼 읽혀 다음 세션이 틀린 전제로 일한다. 이제 컴팩션마다
  색인 전체를 세 질문(행동을 바꾸는가 · 이미 어디 있나 · 완료됐나)으로 재판정한다.
- **우리 스킬은 전부 디렉터리 단위로 등록된다** (#409). 파일 단위 예외 하나가 `references/` 를
  조용히 배포에서 빼고 설치 화면 `skills (N)` 에서도 빠져 있었다. **설치되는 파일은 그대로다** —
  11트랙 × 4CLI 981파일 sha256 대조 diff 0(독립 리뷰 재현). 달라지는 것은 화면의 개수다.

### 설치와 무관한 변경 (이 저장소 운영용)

- **스킬 본문 문구 게이트 5종을 재판정해 55건을 걷었다** (#360~#364, 실행 테스트 99 → 44). 산문의 *뜻*을 어휘로
  판정하려던 시도가 3라운드·598줄까지 가고도 조사 한 글자로 우회당했다. 대신
  `change-management` 에 **"자산은 자기 변경 요청 없이 건드리지 않는다"** 를 신설했다 —
  판정은 읽어서 하고, 딸려 들어간 변경은 `npm run assets:history` 로 찾는다.
- 스킬 descriptor 상한 검사가 자산 하나가 아니라 **전수**로 돈다 (#333).
- npm 게시 인증을 장수 토큰에서 OIDC 로 옮긴 결정을 **ADR-082** 에 남겼다.
- 로드맵을 481 → 287줄로 줄이고 **열린 작업의 정본은 GitHub Issue** 임을 명시했다
  (#337 M1~M4 진척 전수 재판정 포함).
- Finder 산출물 `.DS_Store` 의 git 추적을 끊었다.

## [v26.149.0] — 2026-08-29 (지운 줄 알았던 것, 파일이 아니라 자기가 놓인 자리를 말하는 안내문, 그리고 새 스킬 2종)

### 설치받는 사람에게 달라지는 것

- **`uninstall` 이 codex · OpenCode · Antigravity 산출물까지 실제로 회수한다** (#350). 그전에는
  `.agents/` 가 통째로 남았고 **화면에 언급조차 없었다** — 무엇이 남았는지 알 방법이 없었다.
  실측(v26.148.1): codex 는 `.agents/skills/` 7종, antigravity 는 `.agents/rules/` 6종 잔존.
  `.agents/` 를 삭제 목록에 더하는 방식은 쓰지 않았다 — 그 자리는 `npx skills` 와 공유하고
  그쪽이 스킬 **본문**을 거기 두므로 통짜 삭제가 남의 설치를 지운다. 대신 설치 기록에 있는
  **우리가 쓴 파일만** 지운다: 고친 파일은 남기고 화면에 알리며, 다른 도구의 심링크는
  건드리지 않는다. 이제 codex · OpenCode · Antigravity 셋 다 **잔존 0**(`AGENTS.md` ·
  `opencode.json` 포함). `.mcp.json` 은 종전대로 안내만 한다(사용자 내용이 섞인다).
- **앵커 안내 문장이 자기가 놓인 파일에 대해 참이 된다** (#305). `AGENTS.md` 와
  Antigravity 룰 파일이 *"작동 원칙은 이 파일 맨 아래에서 import 하는 앵커에 있다"* 고
  적고 있었는데, 그 두 파일에는 import 가 없다 — **자기 자신이 앵커**다. codex 만 고른
  사람은 존재하지 않는 파일을 찾게 됐다. (`AGENTS.md` 에는 import 문법이 아예 없다는 것을
  원본 이슈 4건으로 확인했다 — 표준·Codex 양쪽 모두 **미해결 요청**이다.)
- **새 스킬 2종**
  - `self-hosted-github-runner` (#353, 개발 트랙 기본) — GitHub Actions 가 막혔을 때
    **검증을 위조하지 않고** 자기 러너로 같은 검사를 돌리는 절차. 원복까지 한 자리에 남는다.
  - `humanize-korean` (#355, opt-in) — 한국어 글의 AI 티를 **진단해서 근거 있는 곳만** 고친다.
    일괄 치환이 아니라 무엇이 왜 어색한지부터 판정한다.
- **`north-star` 스킬이 방향 · 끝나는 일 · 계속 지킬 것을 갈라 낸다** (#354). 하나로 뭉쳐
  쓰면 "달성하면 추적을 끝내는 것"과 "영원히 지키는 것"이 섞여 로드맵이 썩는다.
- **검증 정책이 "얼마나"와 "언제"로 갈렸다** (#346). 배포되는 룰에서 `Testing` 은 변경의
  위험에 맞춰 **얼마나** 검증할지를, `Delivery` 는 그 결과가 **언제** 있어야 하는지를 말한다.
  검증 수단 메뉴는 상주 룰에서 빼 `verification-loop` 스킬로 내렸다 — 스킬 본문은 매 세션
  실리지 않는다(Codex 의 `AGENTS.md` 예산은 합계 상한이고, 넘으면 설치자 **자기 지시문**이
  조용히 잘린다).
- **OpenCode 도 번들 스킬을 스킬로 받는다** (#381). 커맨드로 변환하던 경로를 폐지했다 —
  같은 스킬을 두 판본으로 받는 일이 없어진다.

### 설치와 무관한 변경 (이 저장소 운영용)

- 이슈 라벨 7종 · 템플릿 3종을 실물로 만들고(#319), 이 저장소를 `gh-issue-workflow` 의
  **적극 단계**로 올렸다(#318). 스킬의 **기본값은 읽기 전용**이다 — 남의 프로젝트에서
  마음대로 이슈를 만들지 않는다.
- 문서가 안내하던 `.opencode/commands/` 가 설치본에 없다는 drift 를 정정했다 (#384).
- 로드맵 M3 의 `uninstall 누락 reverse` 항목을 완료로 갱신하고 카탈로그 수치를 62 로 맞췄다.

### 종결 (하지 않기로 한 것)

- **#349 남은 링크 관통 표면** — 4형태를 전부 재현했지만, **재현은 "코드가 그렇게 동작한다"의
  증거이지 "사용자가 그 상태를 만든다"의 증거가 아니다.** 관측된 링크 생성자는
  `npx skills add` 하나이고 v26.147.1 이 이미 막았다. 실사용자 신고가 오면 재개한다.

## [v26.148.1] — 2026-08-28 (여러 CLI 를 고른 사람이 못 받던 스킬과, 갱신된 적 없던 외부 스킬)

두 결함 다 **exit 0 이고 화면에 ✓ 가 떴다.** 안 된다는 사실을 알 방법이 없었다.

### 설치받는 사람에게 달라지는 것

- **Claude Code 를 다른 CLI 와 같이 골라도 외부 스킬을 받는다** (#372). 두 개 이상 고르면
  스킬이 `.agents/skills/`(Codex · OpenCode · Antigravity 공용)에만 깔리고 **Claude Code 몫의
  `.claude/skills/` 가 조용히 빠졌다.** Claude Code 하나만 고른 사람은 정상이었고 여럿 고른
  사람만 스킬을 못 받았다 — 설치 화면은 ✓ 였다.
- **`update` 가 외부 스킬도 최신판으로 다시 받는다** (#374). 그전에는 몇 번을 돌려도 첫 설치
  판본이 그대로 남았다. 상류 저장소가 스킬을 고쳐도 받을 방법이 재설치뿐이었고, 화면에는
  `external CLI artifacts` 만 떠서 다 갱신된 것처럼 보였다. 이제 **성공 · 실패 · 판정 불가**가
  각각 자기 줄을 갖고, 갱신 중에는 자산마다 진행 줄이 뜬다. 갱신에 실패해도 나머지 update 는
  끝난다. 덤으로, 직접 `npx skills add` 로 깔아 심링크가 된 자리를 실제 디렉터리로 되돌린다.
- **글로벌 설치본의 `update` 가 홈에 쓴다는 사실을 화면이 말한다** (#374). 스코프 표기가
  프로젝트로 고정돼 있어 `~/.claude/` 를 갱신하면서 "no global write" 라고 적었다.

### 설치와 무관한 변경 (이 저장소 운영용)

- 원격 태그 중 npm 에 없는 버전을 매일 묻는 검사를 넣었다 (#367 · ADR-077). GitHub Actions
  장애로 게시가 조용히 빠졌던 것을 하루 안에 잡는다. 의도적 미게시는 CHANGELOG 에 표시한다.

## [v26.148.0] — 2026-08-27 (Claude Code 가 아닌 CLI 를 고른 사람이 못 받던 자산과, 두 판본으로 깔리던 스킬)

같은 위저드에서 같은 자산을 골랐는데 **어느 CLI 를 고르느냐에 따라 실제로 받는 것이 달랐다.**
그 차이는 화면 어디에도 안 나왔다.

### 설치받는 사람에게 달라지는 것

- **Codex · OpenCode · Antigravity 를 골라도 `frontend-design` 을 받는다** (#344). 개발 트랙의
  기본 추천 자산인데 지금까지 이 세 CLI 에서는 **한 번도 설치된 적이 없다.** 배달 방식이
  `claude plugin install` 을 부르는 형태라 구조적으로 Claude Code 전용이었다. `npx skills add`
  경로로 바꿔 네 CLI 전부에 간다.
- **ECC 플러그인을 골라도 같은 스킬을 두 판본 받지 않는다** (#340). `market-research` ·
  `investor-materials` · `investor-outreach` · `nextjs-turbopack` 네 종이 플러그인 사본과 우리
  사본으로 **둘 다** 깔렸고, 어느 쪽이 로드되는지 예측할 수 없었다. 플러그인을 고르면 우리
  사본은 안 깐다.
- **부재를 판정하는 검사기가 "몇 개를 봤는지" 를 말한다** (#329). `check-absence.sh` 가
  `0건`만 내던 것을 검사 대상 수와 함께 낸다 — 대상이 0개라 0건인 것과 실제로 없어서 0건인
  것이 구분되지 않던 자리다. BSD 와 GNU 의 `grep` 이 바이너리를 다르게 세어 같은 트리를
  macOS 500 · Linux 504 로 세던 것도 함께 고쳤다(이제 양쪽 500).
- **배포 룰의 완료 판정 문장이 돌아왔다** (#345). "완료 판정은 만든 레인이 아니라 검증 레인이
  내린다" 가 룰 완화 중 소실됐던 것을 되살렸다.

### 설치와 무관한 변경 (이 저장소 개발용)

룰·스킬 본문의 *뜻*을 CI 로 판정하던 검사를 전부 걷었다 — 같은 뜻으로 다시 쓰면 빨간불이고
어절만 남기면 뜻이 뒤집혀도 초록불이라, 양쪽으로 다 틀렸다(#345 에서 세 라운드 실측). 대신
`npm run assets:history` 가 자산별 마지막 변경 커밋과 **그 커밋이 건드린 자산 수**를 낸다
(#357 · #358). 배포되는 자산은 하나도 바뀌지 않았다.

## [v26.147.1] — 2026-08-19 (다른 도구로 스킬을 받아 둔 프로젝트에서 설치가 죽거나 남의 저장소를 바꾸던 것)

v26.147.0 을 실제로 설치했더니 `npx skills add` 로 스킬을 받아 둔 프로젝트에서 **설치 전체가
실패했다** (#343). `.claude/skills/<id>` 의 실체는 다른 저장소로의 심볼릭 링크인데, 거기에
디렉터리를 부으려던 복사가 죽으면서 고른 자산을 하나도 못 받고 끝났다 — 이후 단계(CLI 변환·
외부 자산·설치 로그)가 통째로 빠진 반쯤 설치된 상태다.

크래시를 없애자 **크래시가 없어 아무도 신고하지 않던 같은 위반들**이 드러났고 함께 막았다.

### 설치받는 사람에게 달라지는 것

- **남의 도구가 소유한 자리 위에서도 설치·업데이트가 완주한다.** 그 자리만 건너뛰고 나머지
  자산은 전부 깔린다. 건너뛴 자리는 화면에 이름과 되받는 방법이 함께 뜬다 — 설치는 성공으로
  끝나므로 화면이 말하지 않으면 고른 자산이 왜 없는지 알 방법이 없다.
- **우리 설치가 사용자의 다른 저장소를 조용히 바꾸던 경로가 막혔다.** 스킬 슬롯 안의 파일·중간
  디렉터리가 링크인 경우, Codex·Antigravity 자리(`.agents/skills/<id>`), 그리고 `update` 가 남의
  저장소 **안에** 우리 파일과 백업을 만들던 경로까지.
- **스킬 자리에 FIFO 가 있으면 설치·업데이트가 영영 멈추던 것**도 막혔다.
- `update` 가 일반 파일 자리에서 `EEXIST` 로 죽던 것도 없어졌다 — install 과 같은 술어를 쓴다.

### 적용 범위 (넓히지 않은 것)

판정 대상은 **스킬 슬롯**(`.claude/skills/<id>` · `.agents/skills/<id>`)이다. `settings.json` ·
`rules/*.md` 처럼 슬롯 밖 파일을 공유 dotfiles 로 링크해 두면 종전대로 링크를 따라 쓴다(백업은
프로젝트 안에 남는다). 같은 술어를 그대로 넓히면 파일 자산이 전부 비디렉터리라 재설치가 통째로
건너뛰어진다. ADR-062 적용 범위 절에 후기로 기록했다.

## [v26.147.0] — 2026-08-17 (위저드가 고르는 것을 보여주고, 이슈를 계층으로 쓰고, 깨진 명령이 못 나가게)

세 갈래가 한 릴리즈에 들어간다 — **설치 화면의 투명성** · **이슈를 백로그로 쓰는 방식** ·
**배포물이 깨진 채 나가는 것을 막는 게이트**.

### 설치받는 사람에게 달라지는 것

- **트랙이 자동으로 고르는 자산이 위저드에 전부 보이고 해제할 수 있다** (ADR-074). 전에는 트랙을
  고르면 **트랙에 따라 18~32개**(`executive` 18 · `tooling` 24 · `full` 32)가 조용히 딸려 왔다 —
  무엇이 깔리는지 화면에 없었다.
- **이번 선택이 밀어낸 자산을 지울지 묻는다** (ADR-075). 트랙을 바꾸면 전 선택의 잔재가 디스크에
  남아 있었고, 그것이 남아 있다는 사실을 알 방법이 없었다.
- **룰이 4개 CLI 전부에 도달한다** (ADR-071). Claude Code 에만 실렸던 룰이 Codex · OpenCode ·
  Antigravity 에도 간다 — "설치했는데 그 CLI 에서는 안 걸린다"가 없어진다.
- **카탈로그 57 → 60**: preline 테마 생성기 · 게임 자산 2종.
- **`mcp-pre-exec` 훅과 `.mcp-allowlist` 제거** (ADR-072) · **ECC 명령 폴백 8종을 기본 설치에서
  제거** (ADR-073). 둘 다 "필요한 틀만 남긴다"는 방향의 실행이다 — 항상 실행되는 조회 차단과
  쓰이지 않는 폴백은 설치받은 사람의 비용이었다.
- **`playwright-launch` 룰이 제거된다** — 내용은 `ui-visual-review` 스킬로 옮겨졌고 게이팅 조건도
  같은 UI 트랙이라 도달은 보전되지만, **층이 상주에서 발화로 내려간다**(브라우저 금지문이 매 세션
  실리지 않고 스킬이 켜질 때 실린다). `update` 가 기존 설치본에서 이 파일을 정리하므로 **안내 없이
  룰 하나가 사라지는 것으로 보일 수 있다** — 그래서 여기 적는다. 룰 7 → 6종.
- `implementer` 에이전트의 frontmatter 가 YAML 로 안 읽히던 것을 고쳤다.

### 이슈를 백로그로 쓰는 방식 (`gh-issue-workflow` 재작성)

- **계층**: GitHub 에 "에픽"은 없다 — 부모 이슈가 에픽이고 하위 이슈가 작업이다. 가용성을 먼저
  재고 쓴다(issue type 은 조직 전용 · Projects 는 토큰 스코프 · 하위 이슈는 `gh` 2.94+).
- **본문을 육하원칙 칸으로** — 관측(`## 문제`)과 추정(`## 제안`)을 분리한다. 준비도 지표는 측정된
  것만 싣는다(`references/readiness.md`).
- **결정은 코멘트에서 본문으로 승격**한다. 코멘트는 스크롤에 묻힌다.
- **재조정은 떼되 지우지 않는다** — 마일스톤·부모만 떼고 이슈는 열어 둔다. 그러지 않으면
  "왜 안 하기로 했나"가 사라져 같은 논의가 다시 열린다.
- **읽기 단계가 기본**이다 (ADR-076). 전에는 opt-in 플래그가 없으면 한 번도 켜지지 않아, 열린
  이슈가 있어도 안 읽었다. 읽기는 원격을 바꾸지 않는다.
- **중간 요청 접수**: 작업 중간에 온 요청이 이번 턴을 넘으면 진행을 멈추지 않고 이슈 초안을
  응답에 붙이고, 승인 후 등록해 번호를 회신한다. 세션·환경 문제는 대상이 아니다.
- **wiki 미러**: 저장소가 wiki 를 쓸 때만, 목표층 문서만, 원본은 `docs/` 에 두고 배너 달린 사본으로.
  첫 페이지 생성은 사용자 행동이다(`gh` 에 wiki 명령이 없다).

### 깨진 것이 못 나가게

- **부정 결론에 대조군을 강제하는 도구**를 배포한다 — `.uzys-agent-harness/check-absence.sh`.
  패턴 부재는 canary 로, 명령 판정은 `--control/--subject` 로 **탐지기가 실제로 무는지 먼저
  보인다**. 빈 결과도 실패한 명령도 "대상이 없다"와 "내 탐지기가 틀렸다"를 구분해 주지 않는다.
- **배포물 셸 건전성 게이트**(저장소 내부). 배포될 `gh` 명령을 줄이음(`\`) 뒤에 주석을 붙여
  깨뜨렸는데 1,501개 테스트가 전부 초록이었다. 검사가 둘인 이유는 하나로 그 결함을 못 잡기
  때문이다 — `bash -n` 은 그 파손을 **문법상 합법으로 통과시킨다**(실측). 게시되는 `.sh` **19개**(`templates/` 18 +
  `package.json` `files` 가 개별 지정한 `scripts/prune-ecc.sh`)는 **문법 검사를 받은 적이 없다.** 일부는 훅·스크립트를 실제로 실행하는 테스트가 있어 간접적으로
  걸리지만, **실행이 도달하지 않는 구간의 문법 오류는 그 테스트들이 잡지 못한다** — 파일 끝에
  오류를 넣으면 기존 스위트가 전부 초록이었다(음성 대조로 확인).

### 문서·규율

- 결정 제시에 **"누구 입장에서 쓰는가"** 축을 넣었다 — 판정표가 코드 관점으로 채워지던 것을 막는다.
- **`docs/CONTEXT-FILES.md` 신설** — 어느 파일이 언제 컨텍스트에 오르는지 한 장으로 본다.
- 룰 전수 조사: 아는 3줄만 압축(`test-policy`).
- **자산 판정의 첫 질문을 목적 적합성으로** 명시했다 — "결정론적인가"·"보안상 좋은가"는 그 다음이다.
  `mcp-pre-exec` 을 "결정론적이다" 하나로 남긴다고 판정했던 것이 사용자에게 적발된 뒤 세운 순서다.

미게시 상태로 쌓인 15커밋을 한 릴리즈로 묶었다.
...
<!-- npm-publish:none 스모크가 구 앵커 경로를 단언해 릴리즈 CI red → needs:ci 가 게시를 막았다. 코드는 v26.141.1 로 나갔다 -->
