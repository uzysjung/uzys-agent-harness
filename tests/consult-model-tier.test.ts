...
`,
    ...
    const started = Date.now();
    const r = run(GEMINI, ["hello"], { AGY_BIN: join(dir, "agy"), GEMINI_CONSULT_TIMEOUT: "1" });
    const elapsed = Date.now() - started;
    expect(r.code).toBe(124);
    expect(r.err).toMatch(/killed after 1s/);
    // 예산 1초 + SIGTERM 뒤 유예 2초 안에 돌아와야 한다 — 3초를 다 기다렸다면 kill 이 안 먹은 것.
