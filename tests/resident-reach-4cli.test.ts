...
describe("각 CLI 의 배선이 실재하는 파일을 가리킨다", () => {
  ...
  it("Codex: 룰이 AGENTS.md 본문에 들어가고 32 KiB 예산에 여유가 남는다", () => {
    ...
    //
    // 2026-08-29 (#346) — 23 KiB → 24.5 KiB. 그 자리에서 물었고 사용자가 정했다.
    //   검증 정책을 머지/배포 게이트로 가르면서 두 룰이 +875 B 커졌다(23,449 → 24,324 B).
    //   **처음 쓴 판본은 +1,554 B 였는데 게이트가 그것을 막았다** — 검증 수단 메뉴(확대
    ...
    //   머지 전 독립 검증 문턱)이 Delivery 룰에 들어가 실측 24,414 → 25,320 B(+906 B: Delivery
    //   +887 · Testing +19). 사용자 문안은 덜어낼 대상이 아니라 같은 변경 안에서 빼지 못했다 —
    //   여유는 24 B 뿐이다. 뺄 것은 정해져 있다: 같은 감사의 F-12(AGENTS 껍데기의 이 리포
    //   메타·이력, 약 −1.5 KB)가 다음 PR 이고, 그때 이 상한을 다시 내린다.
    // 2026-09-13 (감사 F-02 · F-03 · F-05 · F-06 · F-12) — 24.75 KiB → 22.75 KiB. AGENTS 껍데기에서
    //   이 리포의 메타·이력(생성 스크립트명 · 이슈 번호 · 실측 날짜) · 깔리지 않는 에이전트 표 ·
    //   앵커·룰에 없는 무조건 지시("즉시 commit" · 매 세션 SPEC 재참조)를 뺐다. 25,320 → 23,115 B
    //   (−2,205). 예고한 −1.5 KB 보다 크게 내려 위 상향분(+906)을 되돌리고도 남는다.
    const bytes = Buffer.byteLength(body, "utf8");
    const RATCHET = 22.75 * 1024; // 실측 23,115 B 바로 위 (감사 F-12)
    expect(
      bytes,
