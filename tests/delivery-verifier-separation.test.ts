 ...
 *
 * **이 게이트를 믿을 수 있는 범위 — 2026-08-26 확정 (#352)**
 *
 * 무는 것: 계약 문장의 **삭제** · 검사 대상 파일이 목록에서 **빠지는 것** · 완화어 삽입 ·
 * 의무→허가 격하(`-ㄹ 수 있다`) · 길이를 보존한 **전체** 주체 반전.
 *
 * **못 무는 것: 조사를 바꾼 반쪽 반전.** `검증자는 …판정하고` → `구현자도 …판정하고` 로
 * 바꾸면 "구현자도 판정한다"가 되는데 전 스위트가 초록이다(재현 확인). 주체 판정이
 * 주격 조사(이·가·은·는)를 화이트리스트하기 때문이고, 조사·술어 목록을 넓히면 이번엔
 * 정당한 산문이 걸린다.
 *
 * **그래서 더 조이지 않기로 했다.** 세 라운드가 각각 다른 우회로를 찾았고 수렴하지 않았다
 * (①어절 고정 → 동의어 차단·완화 통과 ②완화어 부재 → 모집단 비우면 공회전 ③주체 축 →
 * 조사 우회). 이 게이트의 일은 **소실 방지**이고 그것은 확실히 한다. **의미 반전은
 * 사람(독립 리뷰 레인)이 보는 축이다** — 이 파일이 초록이라고 계약이 온전하다는 뜻은 아니다.
 * 축을 하나 더 넣고 싶어지면, 먼저 위 세 라운드가 왜 실패했는지 읽어라.
 */
