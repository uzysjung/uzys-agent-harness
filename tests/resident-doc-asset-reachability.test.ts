...
describe("상주 문서가 지목하는 자산의 도달 가능성", () => {
  it("지목을 실제로 찾아낸다 (헛통과 차단)", () => {
    ...
    //
    // 2026-09-13 재측정 **1** (ADR-085). 배포 앵커가 전역 6원칙과 바이트 동일해지며 앵커의 스킬
    // 지목 3건이 빠졌다 — 그 지목은 이제 설치기가 **깔린 스킬만** 골라 프로젝트 블록에 쓰는
    // 생성물이라 이 게이트의 모집단(템플릿 파일) 밖이고, 그 도달은 `resident-reach-4cli` 가
    // 실설치로 잰다. 남은 1건은 룰의 지목이다. 하한을 실측 바로 아래로 다시 조인다.
