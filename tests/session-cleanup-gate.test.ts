...
describe("검증 명령의 조용한 실패 — cli-development 소유", () => {
  ...
  it("사고 서사는 룰에 두지 않는다 — 상주 비용을 내는 것은 행동을 바꾸는 문장뿐", () => {
    ...
    // 2026-08-04 (#284) — 판정 범위를 한 섹션에서 **룰 전체**로 넓혔다. 룰이 최소 판단 원칙으로
    // 재작성되며 섹션 헤딩이 사라졌는데, 헤딩으로 잘라내는 판정은 구조가 바뀌는 순간 빈 문자열을
    // 검사하는 공허한 통과가 된다(`not.toBe("")` 가 그걸 잡아 이 테스트가 red 로 알려 줬다).
    // 대상을 좁히는 게 아니라 넓히는 방향이라 약화가 아니다.
    expect(rule).not.toBe("");
    expect(rule).not.toMatch(/\d{4}-\d{2}-\d{2}/);
  });
});
