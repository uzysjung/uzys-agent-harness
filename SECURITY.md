...
2. **Curated third-party assets** — the plugins / skills / workflows the harness installs.
   ...
   isolated environment — a monthly CI job re-runs every install command, and the same verifier
   can be run by hand in a container. Report a vulnerability *inside* an upstream asset to
