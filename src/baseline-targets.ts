 ...
 * 파일 엔트리(`.claude/skills/<id>/SKILL.md`)가 섞여 있었고, 마지막 조각을 쓰면 후자가
 ...
 *
 * **대상은 에이전트만이 아니다** — 스킬이 더 많다. 여기 이름을 옮겨 적지 않는 이유는
 * `listBaselineTargets` 를 트랙별로 불러 보면 그 자리에서 나오기 때문이다 — 목록 사본은
 * 게이트가 늘 때마다 썩는다. 실제로 옛 문안이 "에이전트 4종"으로 열거했다가 #340 이 스킬
 * 4종에 게이트를 붙이면서 틀려졌다.
 *
 * #492 — `withEcc` 가 없어져 "뒤쪽 페이지의 선택을 이 시점에 모른다"는 알려진 한계도 함께
 * 사라졌다. 지금 이 함수가 모르는 opt-in 축은 없다.
 *
