 ...
 *
 * `update` 는 **이미 깔린 파일을 지우지 않는다**(남의 프로젝트 파일을 우리가 삭제하지 않는 현행
 * 규율). 그래서 옛 릴리즈로 깐 설치본에는 이 에이전트들이 그대로 남고, 침묵하면 사용자는 그게
 * 죽은 사본인 줄 모른다. 화면이 대신 말하고 손은 사용자가 — 스킬 쪽
 * `RETIRED_SKILL_IDS`(`external-assets.ts`)와 같은 형태이고, 렌더는 이 목록만 읽는다
 * (`commands/install-render.ts`).
 *
