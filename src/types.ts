...
export interface InstallSpec {
  ...
  /**
   * 2026-08-17 (ADR-075) — 이번 선택이 밀어냈는데 디스크에 남아 있는 자산을 **지울까**.
   *
   * 대표 사례: `ecc-plugin` 을 고르면 하네스의 폴백 에이전트 4종이 manifest 상 설치 대상에서
   * 빠지는데, 이미 깔려 있던 파일은 아무도 안 지워서 같은 일을 하는 에이전트가 두 벌 남는다.
   *
   * **기본값은 거짓**이고 위저드 확인 화면에서만 참이 될 수 있다. 비대화형(`--track` 플래그)
   * 설치에는 물어볼 사람이 없고, 물어보지 않은 삭제는 승인 없이 넘은 경계다.
   */
  cleanSuperseded?: boolean;
}
