...
**ASIS→TOBE 는 "그 결과를 안고 사는 사람" 자리에서 쓴다** (사용자 확정 2026-08-16). 우리가 만드는
것을 **쓰는 사람**, 또는 그것을 운영하는 사람이다. 모듈에 필드가 하나 늘었다는 것은 코드 밖의
누구도 체감하지 못한다 — 표의 항목이 `installer.ts` 나 `resolveRules` 로 시작하면 그 자체가 다시
쓰라는 신호다. 영향받는 역할을 지목하고(설치받는 사용자 / 운영자 / 리뷰어 / 다음 유지보수자),
**전에는 못 하던 무엇을 할 수 있게 되는가** 또는 **무엇이 더는 안 일어나는가**로 적는다.
사용자에게 보이는 변화가 정말 없으면 없다고 하고 실제 수혜자를 적는다 — 지어낸 사용자 이득보다
"이건 다음에 디버깅할 사람만 편해진다"가 낫다.

*이 절이 생긴 이유*: 배포 앵커가 `AS-IS → TO-BE` 를 요구하면서 **누구 입장인지는 한 번도 안
적었고**, 그래서 판정표가 코드 관점으로 채워졌다(사용자 적발). 배포판 대응 절 =
`clear-korean-communication` 스킬 Part 2 (배포 앵커의 §Presenting a decision 은 ADR-085 로 스킬에
흡수됐다 — 앵커는 전역 6원칙과 바이트 동일하다).
...
(ADR-062. 설치·네트워크 불요).
(배포판에서는 `clear-korean-communication` 스킬이 같은 5요소를 싣는다 — ADR-085 가 ADR-055 의 앵커 탑재를 대체.
