...
## 하네스 자산은 자기 변경 요청 없이 건드리지 않는다 (사용자 확정 2026-08-26)

룰 · 스킬 · 훅은 **개별 독립 자산**이다. 하나를 손보러 들어간 작업이 다른 것을 함께 고치지
않는다 — 정리·통일·문체 손질처럼 선의로 하는 일괄 수정이 특히 그렇다.

- **바꾸려면 그 자산 자체를 대상으로 하는 이슈·요청이 있어야 한다.** 다른 작업 중 눈에 띈
  것은 고치지 말고 이슈로 올린다.
- **바꿀 때는 이유와 이슈 번호를 커밋에 남긴다.** diff 는 무엇이 바뀌었는지만 보여 준다.
- **본문의 뜻은 자동 검사로 판정하지 않는다.** 자동화가 맡는 것은 존재 여부와 사본 동기화
  까지다.

**이 절이 생긴 이유**: `ship-checklist` 의 판정 주체 한 줄이 **룰 일괄 정밀화 작업 중 딸려
나가** 사라졌고(#345), 사용자가 직접 발견했다. 그 재발을 막으려 룰 산문을 무는 게이트를
만들었는데 세 라운드 내내 우회당했다 — 어절을 고정하니 동의어 개정이 막히고, 완화어 부재로
뒤집으니 검사 대상을 비우면 공회전했고, 주체 축을 넣으니 조사 한 글자로 샜다. 62줄이
442줄이 되고도 못 막았다. **판정 대상을 내용에서 이력으로 옮기는 것이 답이다** —
"이 문장의 뜻이 뒤집혔나"는 판정 불가이고 "이 자산을 대상으로 한 요청이 있었나"는 판정
가능하다.
