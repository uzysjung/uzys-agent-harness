...
룰 · 스킬 · 훅은 개별 독립 자산이다. **하나를 손보러 들어간 작업이 다른 것을 함께 고치지
않는다** — 일괄 정리 중 한 줄이 조용히 사라지는 것이 이 규칙이 막는 사고다(#345). 눈에 띈
것은 이슈로 올린다. 바꿀 때는 **왜 바꿨는지와 이슈 번호를 커밋에 남긴다**. **본문의 뜻이
옳은지는 읽어서 판정한다** — 에이전트가 변경 이력을 읽고 판정하고, 갈리면 사람이 정한다.
문장의 의미를 무는 자동 검사는 만들지 마라(3회 우회 실측).

| 사례 | 내용 |
|---|---|
| 2026-08-11 | 배포 룰 완화 중 보안 금지문이 절차문으로 격하 — 전 스위트 초록 |
| 2026-08-19 | 그것을 막으려 만든 어휘 게이트가 같은 형태로 실패 |
| 2026-08-26 | 3라운드·598줄까지 키우고도 조사 한 글자로 우회 → 게이트 폐기 |

**딸려 들어간 것을 찾는 방법** (사용자 확정 2026-08-26): `npm run assets:history` — 자산마다
마지막으로 바뀐 날짜·커밋·제목과 **그 커밋이 건드린 자산 수**를 낸다. 자산 수가 크면 일괄
변경이니 그 커밋의 이슈를 열어 **그 자산까지 범위였는지** 본다. 판정은 에이전트가 한다 —
숫자는 읽을 곳을 가리킬 뿐이고, 넓은 정리 커밋은 정당하게 클 수 있다. `--sweep N` 으로
좁히고 `--no-issue` 로 이슈 없이 바뀐 것만 본다.
