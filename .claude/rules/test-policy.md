...
**머지 전 독립 리뷰 문턱** = 핵심 사용자 기능 · 되돌리기 어려운 것 · 돈·권한. 이 리포에서는 **설치자에게 나가는 것**(`templates/` · `src/` · 설치기 동작) · 릴리즈 배선 · 공유 상태 변경이 그것이다. 그 밖(`tests/` · `docs/` · `.claude/` 개발 사본 · 리팩터 · 문구)은 리그레션 테스트로 들어오고 배포 게이트의 독립 검증이 마지막에 본다. 갈리면 독립 리뷰 쪽으로. **검증의 리듬**(#424): 구현 중에는 변경 부분의 빠른 검사만 → 사용자 씬의 수정분이 모이면 독립 검토 → 끝난 뒤 통합 테스트 · 빌드 · 실사용 흐름. 작은 수정마다 전체 검증을 반복하지 않고 재검사는 영향받은 부분만 — 단 문서·자산 변경의 "영향받은 부분"은 아래 §영향 범위대로 전체다.
...
## 가드는 도입 때 한 번만 증명한다 — 가드를 지키는 가드는 두지 않는다 (사용자 확정 2026-09-20, #496)

"안 되게 해라" 한 마디는 **가장 작은 장치 하나**를 산다. 세기는 어조가 아니라 **관측된 발생 횟수 × 피해 × 되돌릴 수 있는가**로 정하고(사다리 = `recurrence-prevention`), 위 변이 대조가 그 **일회** 증명이다. 검사의 대상이 다른 검사·테스트·룰이거나 유일한 위반자가 우리 문서인 검사는 가드가 아니라 문서다(#437 기준 3) — 만들지 않고, 있으면 걷는다(감사 축 = `audit-harness-fit`). 실사례 = #345 · #436 · #478.
