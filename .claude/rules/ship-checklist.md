...
- [ ] **로컬 CI 전체 통과 (1차 게이트)**: `npm run ci`(typecheck + lint + test:coverage + build) exit 0. `npm test` 만으로는 coverage gate 누락. **full 을 요구하는 지점은 여기다** — 커밋에는 테스트가 없고 머지에는 영향 범위만 돈다(시점별 정책 SSOT = `test-policy.md`). **exit code 를 파이프 뒤에서 읽지 마라** — `npm run ci | tail` 의 `$?` 는 `tail` 의 것이고, 백그라운드 실행이면 `;` 뒤 `echo $?` 도 CI 의 코드가 아니다(`cli-development.md` 와 같은 함정)
- [ ] **CI green 이 배포의 전제로 배선돼 있는지**: 릴리스 워크플로가 검증 job 에 `needs:` 로 묶여 있어 **red 면 게시 자체가 안 일어나야** 한다. 현재 `publish` 는 `needs: [ci, docker-e2e]` 다 — `docker-e2e` 는 컨테이너 실설치 3종 — 배달 방식마다 하나(ADR-079) + CLI 를 하나만 골랐을 때의 도달(ADR-080). 확인 없이 태그를 밀지 않는다(신설 근거 = ADR-077 가드레일 G5 · ADR-056: 검증 red 인데 게시가 성공한 릴리즈가 실재한다). **`docker-scenarios.yml`(나머지 시나리오 + 실 CLI)은 게시를 막지 않는다** — 태그마다 자동으로 도니 red 를 눈으로 보고 넘어갈지 정한다
- [ ] **E2E + 커버리지**: 핵심 사용자 흐름 E2E 전부 PASS · `test-policy.md` threshold(이 repo: branches 86)
- [ ] **태그 후 릴리스 CI 확인**: `gh run view <run-id> --json conclusion` 이 `success` (fail 시 patch 태그로 수정). **`gh run watch --exit-status` 를 판정으로 쓰지 마라** — 실패한 run 에 **exit 0** 을 낸 실측이 있다(#377). 종료를 기다리는 데는 써도, 통과 여부는 conclusion 을 읽어서 판정한다. **워크플로가 돌았는지부터 본다** — 발동조차 안 하면 빨간불이 아니라 *아무것도 없음*이라 화면에는 성공한 워크플로만 보이고 게시가 조용히 빠진다(전례 = ADR-077 G5 근거 사고 2026-08-27). 안 돌았으면 `gh workflow run ci --ref <태그>` 로 재트리거
...
- [ ] **보안·의존성**: `npm run security` exit 0 — **baseline 대비 신규 critical/high 0** + `npm audit --omit=dev` critical/high 0. **절대 0 을 요구하지 않는 이유**: 이 저장소가 파는 에이전트는 Bash 를 갖고 훅은 백그라운드 프로세스를 띄운다(설계상 high 29건). 절대 0 은 영원히 성립하지 않아 **통과할 수 없는 게이트가 되고, 그런 게이트는 아무도 안 돌린다**(전례 = #237 · v26.146.0, `docs/plans/rules-hooks-audit-2026-08-16.md`). 신규 findings 를 의도적으로 받아들일 때만 `npm run security:baseline` 으로 baseline 을 갱신하고 사유를 커밋 본문에 남긴다. 이 검사는 릴리즈 CI 의 `ci` job 안에 있어 **red 면 `needs: ci` 가 게시를 막는다**
...
- [ ] **Review 게이트 통과**: 이 릴리즈에 든 문턱 위 변경(`test-policy` §머지 전 독립 리뷰 문턱)이 머지 전 독립 리뷰에서 CRITICAL 이슈 없이 통과했다 — 릴리즈 커밋 자체에는 새 리뷰를 걸지 않는다(ADR-094). **완료 판정은 만든 레인이 아니라 검증 레인이 내린다** — 검증자는 요구사항·변경분·검증 결과를 직접 다시 확인한 뒤 판정하고, 구현자의 보고를 읽고 승인하는 것은 판정이 아니다
...
- [ ] **CHANGELOG 현행화 (🧪 테스트 강제)**: `package.json` 버전을 bump 하면 **같은 릴리즈 커밋에** `CHANGELOG.md` 의 `## [v<version>]` 항목을 추가한다. `tests/docs-supply-chain.test.ts` 의 현행성 게이트가 미기록 시 `npm run ci` 를 실패시킨다(신설 근거 = #196 backfill)
