...
- **부재 확인은 도구로 한다** — `scripts/check-absence.sh --canary <알려진 양성> [-i] <패턴> <경로>...`. canary 를 필수로 받아 **탐지기가 실제로 무는지 먼저 보이고**, stderr 를 보존하고, 파이프 없이 exit code 를 낸다(0 부재 · 1 발견 · 2 신뢰 불가). 위 세 줄이 한 세션에 셋 다 깨진 뒤 도구로 내렸다 — 사람이 매번 기억해야 하는 규약은 규약이 아니다.
