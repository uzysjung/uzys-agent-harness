...
- **스택 기반 큐레이션.** 검증된 옵션 중에서 이 프로젝트에 실제로 필요한 자산만 골라 설치한다. `csr-supabase` 를 고르면 그 track 이 필요로 하는 React, shadcn, Supabase, Postgres 자산과 모든 프로젝트가 공통으로 받는 것들(`find-skills`·`agent-browser`·`frontend-design`, 그리고 개발 track 이면 방법론 스킬)이 미리 체크된다 — 카탈로그의 나머지는 처음부터 끼어들지 않는다.
