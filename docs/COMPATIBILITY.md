...
> **자동 생성** (`scripts/gen-compatibility.mjs`, 2026-06-06). 자산 **38** (official 4 / vetted 30 / experimental 4) · 🟢 검증 **37/38**. tier SSOT=`src/external-assets.ts`, drift 감시=`trust-tier-drift.yml`.
...
| `openspec` | vetted | `@fission-ai/openspec` (npm) | agnostic | 🟢 Docker 2026-06-06 |
| `bmad-method` | vetted | `bmad-method@latest` (npx) | agnostic | 🟢 Docker 2026-06-06 |
| `gsd-orchestrator` | vetted | `get-shit-done-cc@latest` (npx) | agnostic | 🟢 Docker 2026-06-06 |
...
| id | tier | 설치 타겟 | CLI | 검증 |
|---|---|---|---|---|
| `vercel-cli` | vetted | `vercel` (npm) | agnostic | 🟢 registry 2026-06-06 |
| `netlify-cli` | vetted | `netlify-cli` (npm) | agnostic | 🟢 registry 2026-06-06 |
| `supabase-cli` | vetted | `supabase` (npm) | agnostic | 🟢 registry 2026-06-06 |
...
| `agent-browser` | vetted | `agent-browser` (npm) | agnostic | 🟢 registry 2026-06-06 |
...
> uzys-harness(본 하네스 templates)는 외부 자산이 아니라 위 표에 없음 — install-matrix CI + install 테스트로 검증(🟡). 모든 plugin `pluginId`/skill 이름이 실 claude(2.1.167) / `npx skills` 로 **정확성 확정**(content-creator·demand-gen 은 upstream 부재 검출 → v26.76.0 제거).
...
- **검증은 스냅샷**(2026-06-06) — 패키지/마켓 버전은 drift 가능. tier drift 는 `trust-tier-drift.yml` 월 cron 자동 감지, 실설치 재검증은 자산 변경 시 `npm run gen:compat` + Docker 배치.
