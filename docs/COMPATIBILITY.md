...
| Superpowers | `superpowers` | official | plugin `superpowers@claude-plugins-official` | **🟢 2026-06-06** | 실 claude marketplace add + install ✓ |
| ECC | `ecc-plugin` | vetted | plugin `ecc@ecc` | **🟢 2026-06-06** | 199k★. 실 claude install ✓ |
| GSD | `gsd-orchestrator` | vetted | npx `get-shit-done-cc@latest` | **🟢 2026-06-06** | 63k★. registry 1.42.3 + `npx --help` exit 0 ✓ |
| addy agent-skills | `addy-agent-skills` | vetted | plugin `agent-skills@addy-agent-skills` | **🟢 2026-06-06** | 47k★. 실 claude install ✓ |
...
> **워크플로 7/8 🟢** (Docker 실설치 검증). uzys-harness 만 🟡(본 하네스 자체 templates — install-matrix CI + install 테스트로 검증, 외부 실설치 아님). 모든 plugin `pluginId`/marketplace 가 실 claude(2.1.167)로 정확성 확정.
