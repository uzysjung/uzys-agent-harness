...
| `mcp-pre-exec` | MCP allowlist + 위험 패턴(`rm -rf`·`curl\|sh`) **차단** | ✅ | **높다 — 아래 재판정** | **뺀다** (사용자 확정 2026-08-13) |
...
## 훅 판정 정정 — `mcp-pre-exec` 은 뺀다 (사용자 확정 2026-08-13)

**내 1차 판정이 기준을 잘못 골랐다.** "결정론적인가" 하나로 남긴다고 했는데, 사용자가 제시한
기준은 **목적 적합성**이었다 — *"MCP 취약점 때문에 보안 지키겠다고 항상 실행하는 게 우리 제품
취지에 맞나."* 그 기준을 CLAUDE.md §판정은 목적에서 시작한다 에 성문화했다.

목적 기준으로 다시 재니 근거 셋이 전부 제거를 가리킨다:

1. **방향이 반대다.** NORTH_STAR §1 이 *"기준선이 움직인다 — **MCP 추가**와 AI 모델 발전을
   기준에 반영한다"* 고 적어 뒀는데, 이 훅은 MCP 를 추가하는 그 순간 차단한다(미등재 → `exit 2`).
2. **이미 동의한 것에 두 번 묻는다.** `.mcp-allowlist` 는 설치가 `.mcp.json` 서버 목록에서
   **자동 생성**한다. `.mcp.json` 에 넣은 것이 곧 동의인데 같은 사실을 다른 파일에 또 적어야
   통과한다. NORTH_STAR 의 배제 대상(반복되는 실수·심각한 실수)에 해당하는 것이 없다.
3. **위험 패턴 검사는 방향이 뒤집혀 있다.** 실측한 훅 matcher 는 `Write|Edit` 과 `mcp__.*`
   **둘뿐**이고 **Bash 는 아무도 안 본다**. `rm -rf` 가 실제로 도는 자리는 무방비인데, 문서 MCP 에
   그 문자열을 묻는 것은 차단된다 — 보안 경계가 아니라 오탐 생성기다.

**미검증**: 이 훅이 현장에서 무언가를 막은 적이 있는지는 데이터가 없다. 이 저장소는 자기 자신에
미설치라 `hook-blocks.log` 가 없다. 위 판정은 로그가 아니라 설계 근거 3건에 기댄다.

### 제거 범위 (사용자 확정)

- **훅 + allowlist 통째로**: `templates/hooks/mcp-pre-exec.sh` · `templates/settings.json` 배선 ·
  `writeMcpAllowlist`(생성기) · 설치 화면 행 · `templates/mcp-allowlist.example` · 문서 3곳
  (`docs/REFERENCE.md:222` · `docs/USAGE.md:113` · `src/env-files.ts` 헤더). 훅만 빼고 파일을
  남기면 **읽는 사람 없는 게이트**가 되고, 그게 #283 에서 다친 형태다.
- **기존 설치본의 `.mcp-allowlist` 는 `update` 가 정리한다**(사용자 확정). 사용자 편집분일 수
  있으므로 **백업 후 제거**하고 화면에 고지한다.
- 남는 훅은 셋: `protect-files`(차단) · `session-start`(주입) · `task-brief-nudge`(주입).
  **4/4 도달이 목표인 차단 훅은 이제 `protect-files` 하나다.**
