...
| **적대적 입력 테스트** | 정상 흐름 외에 기상천외한 유저 입력·악성 프롬프트(인젝션)를 일부러 투입 | E2E 와 백엔드 가드레일이 **얼마나 빈틈없이** 예외를 방어하는가 |

- [ ] **이름을 갈라 쓴다 (중요).** 사용자가 말한 3층의 셋째를 "변이 테스트"로 부르면 학계 용어와
      충돌한다. 두 개념 모두 우리에게 필요하고 실제로 둘 다 쓰고 있다:
      **ⓐ 소스 변이(mutation testing)** = 구현을 일부러 망가뜨려 *테스트가 무는지* 본다
      — 이 리포가 게이트 검증에 이미 쓰는 "음성 대조"가 이것이다.
      **ⓑ 적대적 입력(fuzzing/red-team)** = 악성·경계 입력을 넣어 *가드레일이 막는지* 본다
      — 사용자가 지정한 셋째 층. 스킬에서 두 이름을 분리해 쓰고, 혼용 금지를 명시
