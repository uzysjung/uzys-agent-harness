...
- [x] **D4** 게이트 `tests/lane-principle-anchor-parity.test.ts` — **테스트 작성 레인 소관**
      (구현 레인은 자기 종료 테스트를 쓰지 않는다). 렌더 산출물 대상 · 앵커 집합 derive · 2성분 판정.
      **커밋 `727d19a` 에 포함**(27 케이스). 전체 파일 채점안은 설계 리뷰가 반증(문장 0개도 초록) →
      **문단 스코프**로 교체
...
- [x] **E6** 게이트 개정 — **테스트 작성 레인 소관**(문단 스코프 채점 · 축2 제거 · 어휘 확장 ·
      ...
      **커밋 `727d19a`** — `where[^.]{0,60}installed` 대안 추가(상한 60자·마침표 불포함의 근거는
      파일 주석). 완화가 아님을 음성 대조로 확인(ack 를 지우면 여전히 빨간불)
- [x] **E7-1** `Rule N` 참조 글롭 스윕 — 상주 문서·배송 표면(`templates/**`·`src/**`) **0건 확인**.
      `docs/**` 의 ADR·plans·specs·archive·research 는 **과거 기록으로 동결**
- [x] **E7-2** `Rule N` 죽은 참조 게이트 신설 — 테스트 작성 레인 소관.
      `tests/resident-rule-reference-liveness.test.ts`(6 케이스), **커밋 `727d19a`**.
      **한계**: `원칙 N`(6원칙 번호)은 아직 무보호 — 재번호 시 참조 5곳이 조용히 죽는다
...
### ②-d 죽은 참조 **런타임 결함** — 리뷰가 놓고 간 M 계열 (2026-07-27)

②의 정리는 **산문 속 죽은 참조**를 다뤘다. 그 리뷰가 남긴 M-1~M-10 중 하나는 산문이 아니라
**실행되는 배선**이었다. 재대조 결과 목록 자체가 또 틀렸다 — **10건 중 4건 기각·1건 하향**이다.
(전례: ②착수 시에도 8건 중 1건이 틀렸다. **에이전트 보고 목록을 그대로 믿지 않는 것이 두 번 다 맞았다.**)

| ID | 판정 | 내용 |
|----|------|------|
| **M-1** | **확정 · 런타임** | `templates/settings.json` PreToolUse 훅이 `.claude/skills/strategic-compact/suggest-compact.sh` 를 **무조건** 참조. `settings.json` 은 `applies: all` 인데 그 스킬은 `!withEcc`(`manifest.ts:153`) → **88 spec 중 44건**(withEcc=true 전량)에서 매 Write/Edit 마다 `bash <없는 경로>` = **exit 127**. 잡는 테스트 0건이었다 |
| **M-3** | 확정 · **M-1 의 거울상** | `agent-introspection-debugging/SKILL.md:142-143` 이 `council`·`workspace-surface-audit` 을 지시. 둘은 upstream(`.dev-references/ecc/skills/`)엔 실재하나 우리가 안 깐다. 참조하는 쪽은 `!withEcc` → **정확히 그때 대상이 없다.** M-1 은 ecc ON 에서, M-3 은 ecc OFF 에서 깨진다 |
| M-2 | **기각** | `templates/mcp.json:14` 가 chrome-devtools 를 정의하고 `composeMcpJson` 이 모든 설치의 base 로 넣는다. 원 보고는 **`src/` 만 뒤진 범위 오류** |
| M-4·M-5·M-6 | 확정 (**전부 HIGH 상향**) | CL-v2 커맨드 표 / impeccable `/teach`·`/shape`·`/polish` / `/uzys:test` 커맨드 자체 부재. M-4 는 upstream lock 충돌이라 **사용자 결정 대기** |
| M-7·M-8·M-10 | **기각** | 수치는 실측 일치(원 보고가 `727d19a` 이전 텍스트를 봄) / `architecture-decision-record` 는 v26.106.0 에서 카탈로그 제거된 이 맥 잔류물 / 양쪽 파일 실재 |
| M-9 | 부분 (LOW 하향) | 설계된 조건부 참조. `strategist.md:57-61` 이 조건 표기 없이 단정하는 것만 결함 |

**M-1 처방 = A′ (런타임 치유기 확장).** 4안 비교는 ADR-057 로 남긴다. 요지만:
템플릿·manifest·**자산 분류는 안 건드린다** — C2→C3 재분류는 Major CR 이고, **ECC 플러그인이 같은
훅을 기본 프로파일로 이미 등록**해(`.dev-references/ecc/hooks/hooks.json`) 항상 설치하면 **이중 발화**가
난다(우리 SKILL.md 자신이 *"two registrations mean the counter advances twice"* 로 금지). 대신 이미 있는
`cleanStaleHookRefs` 의 regex 가 `.claude/hooks/*.sh` 로 좁아 못 물던 것을 `.claude/**` 로 넓히고
**install 경로에서도 1회** 돌린다 — 신규 설치 · **이미 깨진 기존 설치** · 사용자가 스킬을 지운 경우가
한 술어로 덮인다.

- [x] **F1** `keepHookRef` regex·기준 디렉터리·보고 문자열을 원자적으로 확장 + export
- [x] **F2** `installer.ts` 에서 1회 치유(`healStaleHookRefs`) + `InstallReport.staleHookRefs`(**required** —
      optional 이면 새 생성 지점이 조용히 빠뜨린다) + fresh 분기 렌더(**무음 금지가 B안 기각 사유**)
- [x] **F3** 게이트 `tests/settings-reference-parity.test.ts` — 88 spec 을 코드에서 파생, 함의문 1개.
      **면제는 "치유기가 실제로 무는가"로 증명**(그래서 `keepHookRef` export 가 필요했다)
- [x] **F4** 게이트 `tests/publish-needs-verification-gate.test.ts` — §검증정책의 `needs:` 배선.
      글롭으로 훑어 `npm publish` job 을 찾고 ⓐ같은 파일 안 검증 job 에 `needs` ⓑ태그 가드 `if:`.
      변이 3종(needs 제거 · 가드 제거 · 워크플로 분리)이 **영구 canary** 로 상주
- [x] **F5** 구현 레인이 **자기 구멍을 음성 대조로 증명**했다 — `installer` 호출과 fresh 렌더를 **둘 다
      무력화해도 tsc exit 0 + 508건 전부 초록**. 즉 그 두 줄은 지워도 아무도 모른다. 테스트 작성 레인이
      ⓐ렌더(`tests/install.test.ts` — 라벨 + 건수 + **제거 경로 2건이 화면에 나오는지**) ⓑ실 `runInstall`
      경로(`tests/installer.test.ts` 5케이스, 실 `templates/` + 디스크의 settings.json 을 읽어 판정)로 메움.
      음성 대조 2종을 **하나씩 따로** 걸어 각각 정확히 1케이스만 빨간불인 것까지 확인
- [ ] **F6** M-3 처방 미정 — A′ 는 **설정 파일의 실행 참조**를 덮지 콘텐츠 문서의 스킬 지목은 못 덮는다.
      `resident-doc-asset-reachability` 게이트를 `withEcc` 축으로 넓히는 쪽이 후보(그 게이트의
      `specFor()` 가 `withEcc` 를 **항상 false** 로 만들어 M-1·M-3 을 둘 다 놓쳤다)
- [ ] **F7** M-4 사용자 결정 · M-5·M-6 수정 · M-9 조건 표기


### ③ 검증 방법론 자산 — 방법론은 **스킬**로 쓴다

> ⚠️ **2026-07-27 지시가 이 절의 전제 절반을 뒤집었다 — 읽고 나서 착수해라.**
>
> 이 절의 원래 제목은 *"`test-policy` 룰은 **버리고** 방법론을 쓴다"* 였고 근거는
> *"test-policy 는 룰이 필요 없는데 test 방법론에 대해서는 작성했으면 좋겠어"* (2026-07-26)였다.
> 그런데 **2026-07-27 사용자가 확정 검증 정책(커밋/머지/배포 3시점 × 테스트·리뷰 2축)을 주며
> *"너도 명확히 해둬라"* 고 지시했고, 그 표는 `{.claude,templates}/rules/test-policy.md` 에
> 들어갔다**(ADR-056 개정본). 뒤 지시가 이긴다.
>
> **그래서 살아 있는 것과 죽은 것**:
> - **죽음** — *"test-policy 룰을 상주에서 내린다"*. 지금 그 룰은 **확정 정책의 SSOT** 다.
>   지우면 시점별 검증 규약이 사라진다. **다음 세션이 이 절 제목만 보고 지우지 마라.**
> - **살아 있음** — *"방법론(BDD → E2E → 변이 테스트)을 **스킬**로 쓴다"*. 룰은 **언제 무엇을
>   돌리나**(정책)를 답하고, 스킬은 **어떻게 쓰나**(방법론)를 답한다. 둘은 층이 다르므로 공존한다.
>   경계를 그렇게 두면 룰이 방법론 설명으로 부풀지 않는다.

방법론 3층 구조는 사용자가 직접 지정했다:
