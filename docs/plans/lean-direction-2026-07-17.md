...
| 4 | 자산 축 판정 (재정의 #213) | ~~검증실패 12자산 판정~~ **전제 오류로 소멸** — 12 비-🟢 = 🟡 1st-party 템플릿+ecc-prune(실패 아님), 2026-07-17 dispatch 재실행 결과 catalog-verify·trust-tier-drift **양쪽 green**(계측기 RED 방치 해소). 대체 작업: 카탈로그 전수를 `pattern-guide`(모델 대체 가능성 높음 — 축소 후보) vs `operational-fact`(유지) 축으로 분류 → 축소 후보 목록 제안. **제거는 데이터(HITO A/B·설치 로그) + 사용자 컨펌 필수** | ✅ v26.106.0 (62 전수 판정 `asset-axis-judgment-2026-07-18.md` + 사용자 컨펌 — 강등 5·트랙축소 1·제거 1, ADR-035. **큐 ①~⑥ 전체 종결**) |
