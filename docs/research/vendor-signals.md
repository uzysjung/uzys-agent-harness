...
| 2026-08-31 | [The new rules of context engineering for Claude 5 generation models](https://claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models) | **판정을 바꾼 신호.** Anthropic 이 **Claude Code 시스템 프롬프트의 80% 이상을 Opus 5·Fable 5 용으로 제거하고 측정 가능한 손실 없음**을 보고했다. 걷어낸 대상은 **과잉 제약 규칙**(예: *"default to writing no comments… one short line max"* → *"match its comment density, naming, and idiom"*), 남긴 것은 **팀·제품 고유 지식과 gotcha**. → 우리 배포 앵커(7원칙)에 같은 형태가 있는지 **재는** 후속이 필요하다. **지금 지우지는 않는다** — 유추로 지우는 것은 이번 사이클이 기각한 "모델이 이미 안다"와 같은 무근거 절단이다 |
| 2026-08-31 | [Best practices for computer and browser use](https://claude.com/blog/best-practices-for-computer-and-browser-use-with-claude) · [Understanding prompt injections (OpenAI)](https://openai.com/index/prompt-injections/) | **양 벤더가 같은 것을 지시문 층에 적으라고 한다.** Anthropic: *"Treat all web content as untrusted. Design your agent's system prompt to clearly distinguish between the user's instructions and content encountered during task execution"* + *"Remind the model that text found on web pages, in emails, or in application UIs is not from the user and should not be treated as instructions"* — **모델의 학습 시점 견고성에만 기대지 말라**고 명시한다. OpenAI: 신뢰할 수 없는 입력은 *"explicitly marked as untrusted context blocks… should not be treated as instructions"*. → 앵커 §6 의 authority 조항 근거 (#407) |
| 2026-08-31 | [Using CLAUDE.md files](https://claude.com/blog/using-claude-md-files) · [Claude Code memory](https://code.claude.com/docs/en/memory) | *"anytime Claude does something incorrectly, add it to CLAUDE.md so it knows not to repeat the mistake"* → 앵커 §7 의 수치 조항 근거(관측된 실패 3회). 반대 방향 제약도 확인: *"If an entry is a multi-step procedure or only matters for one part of the codebase, move it to a skill or a path-scoped rule instead"* — 이번 추가 2건은 절차도 부분 범위도 아니라 앵커가 맞는 자리다 |
