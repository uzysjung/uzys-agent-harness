...
tier 는 **미리 체크할지 여부를 정하지 않는다** — 그것은 `condition` 이 결정한다. `vetted` 자산 가운데 트랙이
미리 체크하는 것은 4종(`supabase-agent-skills` · `postgres-best-practices` · `react-best-practices` ·
`shadcn-ui`)뿐이고 나머지는 `opt-in` 이다. tier 가 사전 체크에 하는 일은
...
자산별 목록은 여기 두지 않는다(50종 전체 = [COMPATIBILITY.md](COMPATIBILITY.md), 트랙별 묶음 =
...
(#372). ⓒ **`npx-run` 은 `uninstall` 로 되돌릴 수 없다.** 무엇을 어디에 설치했는지 하네스가 추적할 수 없으므로
제거 시 "되돌릴 수 없음"으로 보고한다(옛 설치 로그의 shell-script 항목도 같은 안내를 받는다).

`internal` kind 는 이 표에 없다 — 외부 명령 없이 번들 템플릿을 복사하는 자산이다(번들 스킬 13종 ·
`ci-scaffold` · `tauri-desktop`). 옛 shell-script kind(ECC 전용 스크립트)는 ADR-093 에서 사라졌다. `plugin` 자산은 `claude` CLI 가 PATH 에 없으면 warn-skip 된다.
...
디렉터리 14개. 그중 **13종이 카탈로그 엔트리를 갖고**(`INTERNAL_BUNDLED_SKILL_IDS` — 위저드에서
체크·해제하고 `--with`/`--without` 으로 지정한다) 나머지 1종은 `manifest.ts` 가 트랙 조건에 따라 직접 설치한다.
...
manifest 가 직접 설치하는 1종 — `ui-visual-review`(csr-* · ssr-* · full): Playwright/chrome-devtools 스크린샷 →
baseline diff → 리뷰 게이트. ECC 에서 발췌해 트랙별로 깔던 스킬 7종(python-patterns · python-testing · e2e-testing ·
nextjs-turbopack · market-research · investor-materials · investor-outreach)은 ADR-093(#492)에서 독립 제공을 종료했다 —
모델이 이미 아는 범용 지식이거나 검증 방식을 미리 고정했다. 두 사업용 스킬의 품질 기준은 `strategist` 에이전트로
옮겼고, 옛 설치본에는 `update` 화면이 은퇴를 안내한다(`RETIRED_SKILL_IDS`). cherry-pick 메커니즘(`cherrypicks.lock` ·
`sync-cherrypicks.sh`)은 대상이 0건이 되어 함께 걷어냈다.
...
(ADR-023 · ADR-073 · ADR-081 · ADR-093).
