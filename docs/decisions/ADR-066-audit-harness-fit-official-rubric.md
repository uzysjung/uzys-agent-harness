...
- description 단축은 트리거 정확도 회귀 위험이 있다 — ~~트리거 문구 보존을 테스트로 잠근다.~~
  **(2026-08-30 정정: 이 잠금은 없다.)** ⓐ `audit-harness-fit` 에 붙어 있던 트리거 문구 단언은
  #361 로 걷었다(문구 검사는 정당한 개정을 막고 의미 반전은 통과시켜 양쪽으로 틀린다). ⓑ #333 이
  6종의 description 을 상한 안으로 줄일 때 그 6종에는 트리거 보존 단언이 **처음부터 없었고**
  새로 만들지도 않았다. 독립 리뷰가 그 결과를 실측했다 — `nano banana` · `제3자 관점` · `어색해` ·
  `뭔 소리야` 등 **7개 문구가 자산에서 완전히 소실**됐고, 그중 `external-model-consult` 는
  `templates/CLAUDE.md` 의 상시 발화 목록에 없어 description 이 **유일한 발화 근거**였다.
  → 후속 = 소실된 발화 조건 복원(별도 사이클). **트리거 보존을 문구 게이트로 되돌리지는 않는다** —
  같은 이유로 폐기한 축이다.
