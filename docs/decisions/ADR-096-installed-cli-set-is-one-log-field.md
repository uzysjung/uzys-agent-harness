...
- **기록에 없는 `.claude/` 는 건너뛰되 침묵하지 않는다**(독립 리뷰 BLOCKER-6). v26.125.0 이전에 claude 로
  깔고 다른 CLI 를 추가한 설치본은 그 시절 로그 병합이 없어 앵커 기록이 지워졌고, 유도가 그것을 놓친다 —
  설치자가 직접 만든 `.claude/` 와 기록상 같은 모양이라 기록만으로는 가를 수 없다. 판정은 "건너뛴다"
  (설치자 디렉터리를 지키는 쪽)로 두고, update 화면이 `⊘ .claude/ present but not recorded as installed`
  와 복구 명령(`install --cli claude` 1회 → `clis`·앵커 기록이 굳는다)을 낸다. 디스크 존재는 이 안내를
  낼지 말지에만 쓰인다. update 머리글의 `CLI` 도 깔린 집합을 찍는다(NOTE-F).
