...
- **자기증명(dogfooding)**: 본 자산을 추가하는 과정에서 **A4 stale-count 드리프트 8건을 실제로 발견·정정**했다 — ①`external-assets.ts` "방법론 skill 6종"(실제 7) ②동 파일 `INTERNAL_BUNDLED_SKILL_IDS` 주석 "7" ③`interactive.test.ts` "dev-method skills 6종" ④`docs/COMPATIBILITY.md` 의 **수동** 요약줄(49/59·7종·10) — 자동생성 블록(gen:compat) 바로 위의 수동 수치라는 전형적 drift 구조 ⑤`package-lock.json` 이 v26.83.0 RENAME 이후 4릴리즈째 구 패키지명(`@uzysjung/claude-harness`)+`26.94.0` 로 stale ⑥`README.md` "49/59 assets green today" ⑦`docs/plans/service-audit-roadmap.md` "현 59 자산" ×3 ⑧`adoption-c2-submission-kit.md` "49/59" ×2.
  ⑨`external-assets.ts:164` 헤더 주석 "61 자산 … dev-method skills **6종**" — 78줄 아래 `:242` 의 동일 문자열은 고쳤는데 이건 놓쳤다. **독립 SOD 리뷰가 잡았다**(자체 스윕은 문서만 훑어 놓침). 카탈로그를 정의하는 파일 자신의 주석이 카탈로그 수를 틀리게 말하던 상태이며, 하필 본 ADR 이 자기증명 근거로 삼는 파일이다 — dogfooding 주장이 성립하려면 반드시 정정돼야 했다. 근본 원인 = 총계 게이트가 **문서만** 검사 → `src/` 주석까지 확장(`docs-supply-chain.test.ts`, RED 실증).
  ⑥⑦⑧ 은 **본 PR 자신이 만든 드리프트**였고 기존 게이트가 잡지 못했다 — `docs-supply-chain.test.ts` 의 총계 게이트가 `index.html`·`COMPATIBILITY.md` 만 검사했기 때문. no-false-ship("동일 목록이 2곳 이상 하드코딩 → derive 단일화 또는 exhaustiveness 테스트 없이 머지 금지")에 따라 **게이트를 README·로드맵·제출 kit 까지 확장**했다. 스킬이 주장하는 검사 유형이 이 repo 에 실재함을, 스킬을 추가하는 작업 자체가 입증한 셈.
...
- **부정/리스크**:
  (z) **안전(SAFETY) 축 부재 — 알려진 갭** (독립 SOD 리뷰 Important #3, 2026-07-16). A/B/C 는 **위험한 하네스를 통과시킨다**: unpinned `curl | sh` 훅이나 permission prompt 를 건너뛰라는 룰은 TRUE(정확)+USED(작동)+AFFORDABLE(한 줄) → 감사 clean. 린터 위임분은 **form** 안전(시크릿·SHA-pin)만 덮고, "살아있고 정확한 지시가 나쁜 생각인가"는 정확히 "린터가 구조적으로 못 하는 판단" = 본 스킬이 자기 존재 이유로 선언한 영역이다. 초판은 그 상태로 "Every check belongs to exactly one"·"covers both" 라는 **완전성을 단언**했다 — 택소노미가 뒷받침 못 하는 주장. 처리: 완전성 단언 철회 + 안전을 A/B/C 위 **상시 렌즈**로 명시 + 리포트에 `Safety concerns` 행 강제(빈 값은 "none" 이 아니라 **"none seen — not a clean bill"**: 체계적 pass 가 없으므로 침묵은 증거의 부재이지 부재의 증거가 아니다). **4번째 축 신설은 후속 여지** — 본 릴리즈는 갭을 정직하게 표기하는 선까지.
