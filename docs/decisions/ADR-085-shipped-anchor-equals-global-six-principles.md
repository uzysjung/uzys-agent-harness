...
- Amended by: ADR-088 (`CONTINUOUS_SKILLS` 3종 → 2종 — 브리프 스킬이 빠졌다. 안내를 설치기가
  프로젝트 블록에 쓰는 결정은 그대로) · ADR-091 (결정 1 의 "전역 파일과 바이트 동일"이 끝난다 —
  앵커는 #463 첨부 Working Principles 문안. 결정 2·3·4 는 그대로)
