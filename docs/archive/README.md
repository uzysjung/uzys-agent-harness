...
| `spec-foundation-v26.38.md` | **직전 `docs/SPEC.md`** — Foundation(v26.38) 시점의 Persistent Anchor. `DO NOT CHANGE 본문` 으로 보존돼 있었다 | 2026-07-29 사용자 승인으로 **내용 그대로 이동**. 옮긴 이유는 본문이 *"현재 목표로 읽지 말 것"* 이라 적혀 있는데 SessionStart 훅이 매 세션 "이걸 먼저 읽어라"고 안내해 **안내가 자기부정 문서를 지목**하고 있었기 때문이다. 내용 불변은 git 이력이 보증한다(해시 고정 테스트는 2026-09-21 #454 로 퇴역) |
