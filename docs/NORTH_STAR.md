...
> **지시문 8개 ~4,452**(적정성 축 — 조용한 증가만 차단) · **발화 표면 23개 ~3,233**(유지 축, 스킬별 상한만) ·
> 합계 31개 ~7,685 이 설치자가 실제로 무는 전부다.
> 내역: rules 6개 ~1,495 · CLAUDE.md 2개 ~2,957 · skill 14개 ~2,467 · agent 9개 ~766.
...
> *깎으면 안 불린다*로 판단이 다르다. 참고로 지시문의 66%가 CLAUDE.md 다(2,957 / 4,452).
