...
> **지시문 8개 ~4,481**(적정성 축 — 조용한 증가만 차단) · **발화 표면 26개 ~3,409**(유지 축, 스킬별 상한만) ·
> 합계 34개 ~7,890 이 설치자가 실제로 무는 전부다.
> 내역: rules 6개 ~1,462 · CLAUDE.md 2개 ~3,019 · skill 17개 ~2,643 · agent 9개 ~766.
...
> *깎으면 안 불린다*로 판단이 다르다. 참고로 지시문의 67%가 CLAUDE.md 다(3,019 / 4,481).
