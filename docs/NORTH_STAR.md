...
> **지시문 8개 ~4,379**(적정성 축 — 조용한 증가만 차단) · **발화 표면 26개 ~3,524**(유지 축, 스킬별 상한만) ·
> 합계 34개 ~7,903 이 설치자가 실제로 무는 전부다.
> 내역: rules 6개 ~1,360 · CLAUDE.md 2개 ~3,019 · skill 17개 ~2,799 · agent 9개 ~725.
> (2026-09-13: CLAUDE.md +65 — 배포 앵커가 사용자 전역 6원칙과 바이트 동일해졌다(ADR-085, 앵커 −513 B)
> 그리고 앵커에서 빠진 상시 스킬 안내를 설치기가 프로젝트 블록에 쓴다(전 스킬 선택 기준 상한으로
> 계상). 사용자 결정에 따른 의도적 갱신. 직전 지시문 ~4,314 · CLAUDE.md ~2,954 · 합계 ~7,838.)
>
...
> *깎으면 안 불린다*로 판단이 다르다. 참고로 지시문의 68%가 CLAUDE.md 다(2,954 / 4,314).
