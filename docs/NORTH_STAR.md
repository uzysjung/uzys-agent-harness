...
> **현재 상태 (2026-08-03, spec-drift 스크립트 복원 후)**: **상주 + 발화 양축 계측 완료** — `npm run cost:report`.
> 실측(tooling 트랙): **상주 23개 항목 · ~5,837 tokens/세션** = rules 6개 ~2,430 ·
> CLAUDE.md 2개 ~2,391 · agent descriptors 9개 ~724 · skill descriptors 6개 ~292.
> ADR-061 다이어트 직후는 23개 ~5,715(rules ~2,308)였다 — 직전(ADR-060 정비 직후 24개 ~6,552)
> 대비 −1개 · −833 tok, `gates-taxonomy` 삭제(룰 9종→8종)와 잔여 8종의 절 단위 감량(양 사본 합계
> 1,100 → 535줄)의 결과. 그 앞(30개 ~7,570)부터 세면 −7개 · 약 −1,850 tok. 이후 spec-drift-check
> 스크립트 복원(#275, 룰 2종에 실행 지시 문안)이 +122 tok 을 되샀다 — 게이트 없는 프로즈로
> 남겨둔 값을 실행 게이트로 바꾼 대가다.
> **개수는 1 만 줄었는데 토큰이 833 줄어든 것이 다이어트 사이클의 형태다** —
> 파일을 지운 것이 아니라 남긴 파일에서 모델 기지식 교본을 걷어냈다(부수 축 둘이 서로의 굿하트를
> 막는다는 아래 설계가 실제로 갈라 보인 첫 사례).
