...
| **상주의 근거** | 지시는 설치자의 작업이 그것 없이보다 빠르고 안정적이라는 **관측**을 근거로 상주한다 — 크기가 아니라 관측이 판정한다. 관측이 없는 지시는 상주에서 빠져 스킬(발화 시에만 비용)로 내려가거나 은퇴한다 |

*(메인테이너 확정 2026-07-27. 배제 대상은 **반복되는 실수·심각한 실수 둘뿐**이고, 남기는 것은
위 다섯이다. 이 범위를 넘는 자산은 사유를 대야 한다 — §4.1 상주 입증 책임.)*

*(상주의 근거 행은 메인테이너 확정 2026-09-14 · #452. 첫 전수 적용 = ADR-090 — 관측 없는 스킬 4 · 에이전트 3
은퇴, 룰 44문장 중 프로즈가 행동을 바꿨다는 긍정 관측 0. 판정 표 = `docs/plans/harness-inventory-audit-2026-09-14.md`.)*
...
> **현재 상태 (2026-09-20, ADR-083 축 분리 뒤)**: `npm run cost:report` 실측(tooling 트랙) —
> **지시문 8개 ~3,770**(적정성 축 — 조용한 증가만 차단) · **발화 표면 12개 ~2,175**(유지 축, 스킬별 상한만) ·
> 합계 20개 ~5,945 이 설치자가 실제로 무는 전부다.
> 내역: rules 6개 ~1,501 · CLAUDE.md 2개 ~2,269 · skill 10개 ~1,983 · agent 2개 ~192.
> (2026-09-20 ⓛ rules +233 · skill +21 — Testing 룰의 원칙을 "필요한 보호는 유지하고 검증 방법은 근거에
> 따라 개선"으로 바꿨다(#496, 사용자 문안): 가드는 개수·단계가 아니라 추가 검증 가치와 총부담으로 고르고,
> 테스트는 기대값이 틀렸으면 고칠 수 있다. 재발 방지 스킬의 descriptor 도 횟수별 강제 승격에서
> 근거 기반 재평가로. 사용자 결정에 따른 의도적 갱신. 직전 지시문 ~3,537 · 발화 표면 ~2,154 · 합계 ~5,691.)
> (2026-09-20 ⓚ CLAUDE.md +31 · skill +12 — `clear-korean-communication` → `user-centered-explanation`
> 개명(#485) + 상시 안내 한 줄을 사용자 원칙 문안으로 교체(#493: 내부 구현보다 사용자 행동·결과를
> 먼저). descriptor 는 사용자 개정안. 사용자 결정에 따른 의도적 갱신. 직전 지시문 ~3,506 ·
> 발화 표면 ~2,142 · 합계 ~5,648.)
> (2026-09-20 ⓙ skill +19 — `audit-harness-fit` 3판(#462): descriptor 가 "정리·제거" 에서 "자율 선택·
> 생산성·품질" 로 발화 조건을 넓혔다(114 → 133). 사용자 문안에 따른 의도적 갱신. 직전 발화 표면
> ~2,123 · 합계 ~5,629.)
> (2026-09-20 ⓕ CLAUDE.md −719 — 배포 앵커를 사용자의 Working Principles 문안으로 교체(#463,
> ADR-091). 사용자 결정에 따른 의도적 갱신. 직전 지시문 ~4,225 · 합계 ~6,348.)
> (2026-09-20 ⓔ skill −198 — `clear-korean-communication` 을 사용자 문안으로 교체(#465): descriptor 가
> 짧아졌다. 사용자 결정에 따른 의도적 갱신. 직전 발화 표면 ~2,321 · 합계 ~6,546.)
...
> 따른 의도적 갱신. 직전 지시문 ~4,314 · 발화 표면 ~3,524 · 합계 ~7,838.)
> (2026-09-13 ⓒ rules +102 · agent +41 — 리뷰 문턱 통일(ADR-087, #424 · #423): Delivery 룰에 씬 단위 검증
> 리듬과 머지 전 독립 검증 문턱이 들어갔고 `code-reviewer` descriptor 가 "매 변경" 에서 씬 완료·문턱 위로.
> 사용자 문안에 따른 의도적 갱신. 직전 지시문 ~4,379 · 발화 표면 ~3,368 · 합계 ~7,747.)
> (2026-09-13 ⓓ skill −3개 −176 · CLAUDE.md −62 · rules +33 — 자산 정리(ADR-088, #426 F-04·09·10·11):
> 스킬 3종 은퇴(컴팩션 타이밍 · 세션 관측 · SPEC 분리)로 descriptor 3개가 빠지고, 브리프 스킬이
> 상시 스킬 안내에서 빠져 프로젝트 블록이 한 줄 줄었다. 룰은 `doc-governance` 에 상주 문서 한 줄이
> 늘었다(은퇴한 SPEC 분리 스킬의 대체). 사용자 결정에 따른 의도적 갱신. 직전 지시문 ~4,481 ·
...
> (2026-09-14 ⓘ skill −4개 −338 · agent −5개 −375 · rules −105 — 전수 판정 적용(ADR-090, #452):
> 관측 없는 스킬 4종(`verification-loop` · `deep-research` · `eval-harness` ·
> `agent-introspection-debugging`)과 에이전트 3종(`plan-checker` · `silent-failure-hunter` ·
> `build-error-resolver`) 은퇴, `data-analyst` · `strategist` 는 도메인 트랙 조건부로 강등(tooling
> 에서는 안 깔린다 — full 에서는 그대로 받는다). 룰은 앵커와 중복이거나 상주 중에도 못 막은 문장을
> 걷었다. 사용자 확정에 따른 의도적 갱신. 직전 지시문 ~4,330 · 발화 표면 ~3,034 · 합계 ~7,364.)
>
...
> *깎으면 안 불린다*로 판단이 다르다. 참고로 지시문의 70%가 CLAUDE.md 다(2,957 / 4,225).
...
**감산은 곧 대체다.** 빼기만 하는 것이 목표가 아니다 — 범용 프로세스 거버넌스를 덜어낸 자리에 **원칙과 방법론의 깊이**를 넣는다(메인테이너 지시: *"원칙/방법론이 더 중요할 듯"*). 실측된 공백: `templates/` 전수에서 속성기반·계약 테스트·FDD 는 **0건**, 변이 테스트는 언급만 4건이고 선택 가이드가 없다. 검증 절차를 싣던 스킬은 CI 실행 순서만 담고 "어느 검증이 이 상황에서 무는가"는 다루지 않아 ADR-090 에서 은퇴했다 — 공백은 그대로다. 이 추가는 같은 기준의 정상 결과다 — 필요한데 없으면 넣는 것이고, 입증 책임을 **사고 기록**이 진다(초록불이 안 무는 테스트를 반복 제출해 `no-false-ship` 에 "음성 대조" 절을 신설해야 했던 전례). 범위는 **선택 규칙과 함정**으로 한정한다. "TDD 가 무엇인가"는 모델이 이미 안다.
