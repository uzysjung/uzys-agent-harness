...
> **현재 상태 (2026-09-14, ADR-083 축 분리 뒤)**: `npm run cost:report` 실측(tooling 트랙) —
> **지시문 8개 ~4,225**(적정성 축 — 조용한 증가만 차단) · **발화 표면 12개 ~2,321**(유지 축, 스킬별 상한만) ·
> 합계 20개 ~6,546 이 설치자가 실제로 무는 전부다.
> 내역: rules 6개 ~1,268 · CLAUDE.md 2개 ~2,957 · skill 10개 ~2,129 · agent 2개 ~192.
