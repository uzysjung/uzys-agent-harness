...
> 실측(tooling 트랙): **상주 ~6,174 tokens/세션** = rules ~4,071 · CLAUDE.md ~938 ·
> agent descriptors ~615 · skill descriptors ~550. 발화 = 기본 설치 8종 전부 트리거 시 ~26,930.
> **2026-07-19 하루에 5,194 → 6,174 (+19%)** — 전부 룰 증분이다(재발방지 3건 + no-false-ship
> 배송 결정). 각 증분은 측정된 실패에 근거하지만 방향은 ratchet 역행이며, ADR-043 2단계 eval 의
> 1순위는 여전히 룰 계층이다.
>
> 마지막 증분(v26.127.0, +99)은 이 게이트가 **초안을 되돌려 깎게 만든 첫 사례**다 — 처음 쓴
> 분량이 +152 였고, 배포판(`templates/`)만 조이고 상세 서술은 이 리포 사본에 남겨 53 을 줄였다.
> 계측이 없었으면 셋 다 그냥 실렸다.
