...
> 실측(tooling 트랙): **상주 23개 항목 · ~4,945 tokens/세션** = rules 6개 ~1,063 ·
> CLAUDE.md 2개 ~2,866 · agent descriptors 9개 ~724 · skill descriptors 6개 ~292.
> 직전(2026-08-10, 23개 ~4,968) 대비 **−23 tok** — 룰 −29, 앵커 **+6**.
> 중복 제거만 하면 −102 였으나, 같은 사이클에서 사용자 리뷰로 **룰 5종의 과잉 강제를 완화**하며
> 문장이 다시 늘었다(−102 → −29). 완화한 것: 고위험 변경에 모든 경로 테스트를 강제하던 표현 ·
> "외부 서비스는 항상 mock" 으로 읽히던 문장(우리 자신의 Dev-Prod Parity 와 충돌) · 파이프라인을
> 고치라고 명령하던 ship 조항 · 이미 합의된 범위 안에서도 승인을 반복 요구하던 조항 · 스스로
> 모순이던 보호장치 절(승인 원칙을 세워 놓고 "dry-run 하고 적용한다"고 시켰다).
> **총량은 지표가 아니다** — 잘못 강제하는 짧은 문장보다 정확한 긴 문장이 낫다.
...
> 그 앞으로 세면 30개 ~7,570 → 23개 ~4,945, 즉 **−7개 · 약 −2,625 tok**.
