...
- **차단은 기록을 남긴다** (2026-07-27 · ADR-058 → **2026-08-02 구현 · ADR-061**) — exit 2 로 막을 때마다 한 줄(`날짜 · 훅 · 대상 · 사유`)을 `.uzys-agent-harness/hook-blocks.log` 에 남긴다. 차단하는 훅 전부가 그렇게 한다 — 배포판 `protect-files`·`mcp-pre-exec`, 이 리포의 `docker-only-realcli`. 로그 쓰기가 실패해도 차단 판정(exit 2)은 바뀌지 않는다. **아직 증거가 아니라 계측기다** — 누적치 0 에서 시작하므로 "무엇이 실제로 막는가"는 다음 감사에서 처음 답해진다. 이 로그가 다음 감축의 유일한 증거원이다
