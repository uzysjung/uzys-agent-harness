...
> **"설치한 프로젝트에서 에이전트가 그 저장소의 사실을 알고 시작하게 하고, 틀린 것은 만든 레인이 아니라 게이트와 독립 레인이 잡게 한다 — 설치는 그 최소 구성을 4개 CLI 에 배달하는 수단이다."**

목적은 **개발 속도를 지키면서 높은 품질을 안전하게 딜리버리하는 것**이고, 그 수단은 룰이 아니다.
두 축으로 나뉜다 — **속도**는 에이전트가 앵커의 사실을 믿고 바로 일할 수 있을 때 나오고(사실이
틀리면 없는 훅을 찾고 없는 명령을 돌리는 왕복이 된다), **품질**은 만든 사람이 아닌 것이 틀림을
잡을 때 나온다.

> **왜 "룰이 막는다"가 아닌가.** 전량 귀속 가능한 사고 기록에서 **주어가 룰·프로즈면 검출 0건**,
> 게이트·테스트·독립 레인·사용자면 7건이다. 반례가 결정적이다 — `cli-development` 가 세 가지를
> 이미 규정하고 있었는데 같은 세션에서 셋 다 깨졌고 3회차는 거짓 보고가 됐다. 룰이 실제로 바꾼
> 것은 예방이 아니라 **공시**였다. 벤더 문서도 같은 선을 긋는다: *"Claude treats them as context,
> not enforced configuration. To block an action, use a PreToolUse hook instead."*
> 그러므로 룰을 쓰지 말라는 것이 아니라 — **필요하면 쓰되, 그것이 낡으면 빨간불이 되게 한다.**
