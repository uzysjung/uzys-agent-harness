...
> **지시문 8개 ~3,668**(적정성 축 — 조용한 증가만 차단) · **발화 표면 12개 ~2,154**(유지 축, 스킬별 상한만) ·
> 합계 20개 ~5,822 이 설치자가 실제로 무는 전부다.
> 내역: rules 6개 ~1,399 · CLAUDE.md 2개 ~2,269 · skill 10개 ~1,962 · agent 2개 ~192.
> (2026-09-20 ⓛ rules +131 — Testing 룰에 "안 되게 해라 한 마디는 최소 장치 하나 · 가드는 도입 때 한 번만
> 증명 · 가드를 지키는 가드는 두지 않는다" 한 항목(#496, 사용자 지시). 룰이 늘어난 대가로 2차 가드가 생기는
> 것을 막는다. 사용자 결정에 따른 의도적 갱신. 직전 지시문 ~3,537 · 합계 ~5,691.)
