...
> 실측(tooling 트랙): **상주 23개 항목 · ~4,908 tokens/세션** = rules 6개 ~1,032 ·
> CLAUDE.md 2개 ~2,860 · agent descriptors 9개 ~724 · skill descriptors 6개 ~292.
> 직전(2026-08-10, 23개 ~4,968) 대비 **−60 tok** — 앵커가 이미 소유한 사실 3건을 룰에서 걷어냈다
> (#289. "실행하지 않은 검사는 통과가 아니다"는 **3중**이었다). **앵커는 한 글자도 안 줄였다**:
> 룰은 4 CLI 중 2곳(Claude Code·OpenCode)에만 도달하고 앵커는 4곳 전부에 가므로, 앵커에서 지우면
> Codex·Antigravity 설치자는 그 지시를 통째로 잃는다. 추상↔구체로 갈리는 것(기준 약화 금지)은
> 중복이 아니라고 판정해 남겼다.
>
...
> 그 앞으로 세면 30개 ~7,570 → 23개 ~4,908, 즉 **−7개 · 약 −2,662 tok**.
