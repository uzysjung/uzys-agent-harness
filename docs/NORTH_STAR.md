...
> **현재 상태 (2026-08-02, ADR-060 정비 후)**: **상주 + 발화 양축 계측 완료** — `npm run cost:report`.
> 실측(tooling 트랙): **상주 24개 항목 · ~6,552 tokens/세션** = rules 7개 ~3,141 ·
> CLAUDE.md 2개 ~2,395 · agent descriptors 9개 ~724 · skill descriptors 6개 ~292.
> 정비 전(30개 ~7,570) 대비 −6개 · 약 −1,000 tok — 스킬 외부화(ADR-060)와 룰 21→9 의 결과다.
...
- **원칙에는 만료 시계가 붙는다 (sunset)** — 모델이 좋아지면 완화해야 하지만 **"나중에 재평가"는 이 리포에서 한 번도 실행된 적이 없다**(HITO 3개월·판정 1회 / JAR 8일·0회 / §4.3 "분기 1회 재평가"는 한 분기가 지나도록 숫자조차 안 고쳐졌다). 그래서 **임계값을 나중에 정하는 대신 입력부터 만든다**: 상주 항목 줄에 `근거(ADR/PR/커밋) · 측정일 · 모델` 3필드를 붙이고, 없으면 게이트가 **"미측정"으로 표시**한다(차단 아님 — 옥죄임을 늘리지 않는다). 필드 없이 미루면 재평가가 영원히 시작되지 않는다. 현재 33개 룰 파일에 모델 좌표는 **0건**이다
- **차단은 기록을 남긴다** (2026-07-27 · ADR-058) — exit 2 로 막을 때마다 한 줄(`날짜 · 훅 · 대상 · 사유`)을 남긴다. 지금은 차단 지점 7곳이 **로그 0줄**이라 무엇이 실제로 막고 있는지 아무도 모르고, 그래서 "옥죈다"가 느낌 대 느낌으로 남는다. 이 로그가 다음 감축의 유일한 증거원이다
...
| 선택 | 포기한 것 | 근거 |
|------|----------|------|
| 룰 slim-down (실측 2026-07-27: `.claude` 12개 729줄 / `templates` 21개 974줄) | 범용 guide 두께 | 어휘는 정확성 > 분량. 린터 가능한 건 린터에. **"분기 1회 재평가"는 한 번도 발동하지 않았다** — 위 sunset 3필드가 그 대체물이고, 이 줄의 수치도 그때 같이 갱신된다 |
