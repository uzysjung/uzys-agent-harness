...
> 실측(tooling 트랙): **상주 23개 항목 · ~4,725 tokens/세션** = rules 6개 ~1,062 ·
> CLAUDE.md 2개 ~2,647 · agent descriptors 9개 ~724 · skill descriptors 6개 ~292.
> 직전(2026-08-03, 23개 ~5,837 · rules ~2,430) 대비 **−1,112 tok, 개수는 그대로** — #284 가 룰에서
> 고정 수치·도구별 명령·다단계 절차·템플릿을 걷어내 판단 원칙만 남긴 결과다(rules 2,430 → 1,062).
> 같은 사이클에 앵커가 +256 되돌아왔다(#281, 상시 적용 스킬 3종의 발화 지점) — 스킬은 설치돼
> 있어도 발화 지점이 없으면 descriptor 값만 내고 안 돈다. UI 트랙은 `benchmark-parity` 가 빠져
> 항목도 −1 이다(26→25).
> 그 앞으로 세면 30개 ~7,570 → 23개 ~4,725, 즉 **−7개 · 약 −2,845 tok**.
