...
> 실측(tooling 트랙): **상주 23개 항목 · ~4,455 tokens/세션** = rules 6개 ~1,048 ·
> CLAUDE.md 2개 ~2,391 · agent descriptors 9개 ~724 · skill descriptors 6개 ~292.
> 직전(2026-08-03, 23개 ~5,837 · rules ~2,430) 대비 **−1,382 tok, 개수는 그대로** — #284 가 룰에서
> 고정 수치·도구별 명령·다단계 절차·템플릿을 걷어내 판단 원칙만 남긴 결과다(rules 절반 이하로).
> UI 트랙은 `benchmark-parity` 가 빠져 항목도 −1 이다(26→25, ~6,515→~4,744).
> 그 앞으로 세면 30개 ~7,570 → 23개 ~4,455, 즉 **−7개 · 약 −3,100 tok**.
