...
- **이 위계는 있는 문서의 역할표이지 없는 문서를 만들라는 지시가 아니다.** README 하나면 되는 프로젝트가 정상이다.
...
- 신규 기능 → **SPEC 이 있는 프로젝트면 등재가 먼저, 구현은 그 다음**(spec-first). "추가하자"는
  문서 작업 지시이지 구현 착수 신호가 아니다.
...
`CLAUDE.md` 원칙 1("읽고 나서 쓴다")을 **문서가 미완이라 표기한 항목**에 적용한 것 — 여기서는
...
**본 규약을 무는 게이트는 기본 제공되지 않는다** — 안 지켜져도 아무도 안 막는다. 같은 drift 가
반복되면 **게이트를 만들어라, 프로즈를 늘리지 말고.** 무엇을 검사할지는 프로젝트의 문서
레이아웃이 정한다(어느 파일의 미완 항목·Status 를 셀 것인지).
